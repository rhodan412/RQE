...
function RQE:OpenSettings()
	...
	-- Open the actual AceConfig panel registered by Core.lua. Passing its
	-- display-name string opens the generic Game settings panel on Classic Era.
	-- if self.optionsFrame then
	-- 	if SettingsPanel and SettingsPanel.OpenToCategory then
	-- 		SettingsPanel:OpenToCategory(self.optionsFrame)
	-- 		return
	...
	-- Modern Classic's Settings API requires the registered category's ID, not
	-- its canvas frame. Look up the category created by AceConfigDialog first.
	if self.optionsFrame then
