...
function RQE.getQuestData(questID)
	...
	-- Select only the database sections that use map IDs for the running client.
	-- Retail retains the original expansion-first lookup order. Anniversary and
	-- SoD clients first check their dedicated override table, then fall back to
	-- the original expansion data for quests that have no client-specific entry.
	...
	local function GetRetailDatabaseOrder()
		...
		-- Keep Retail data ahead of all older Retail sections.  This is important
		-- when a reused quest ID has expansion-specific Retail map IDs.
	...
	-- local databaseOrder = majorVersion == 2
	-- 	and { "BurningCrusade", "Vanilla" }
	-- 	or { "Vanilla" }
	local databaseOrder
	if majorVersion >= 4 then
		-- Retail: never use Anniversary/SoD sections, because their map IDs are
		-- intentionally specific to the Classic-era clients.
	...
	elseif majorVersion == 3 and (minorVersion > 5 or (minorVersion == 5 and patchVersion > 0)) then
		...
		-- Wrath Anniversary (> 3.5): only the relevant Anniversary-era tables.
	...
	elseif majorVersion == 2 and minorVersion >= 5 then
		...
		-- Burning Crusade Anniversary (2.5+): Anniversary and SoD data only.
	...
	elseif majorVersion == 1 and minorVersion >= 15 then
		...
		-- Season of Discovery (1.15+): SoD data only.
	...
	--[[
	-- Legacy multi-expansion lookup retained for reference only. Classic must
	-- not execute this section because it can select non-Vanilla database data.
	...
	local wodGarrisonLevel = C_Garrison and C_Garrison.GetGarrisonInfo and C_Garrison.GetGarrisonInfo(2)
