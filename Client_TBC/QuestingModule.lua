...
QuestingModule.lua
TBC quest tracker frames, search, sorting, scenario displays, and interactive quest controls
...
--------------------------------------------------
	...
	local objectiveTrackerWatchdogElapsed = 0
	objectiveTrackerWatchdog:SetScript("OnUpdate", function(_, elapsed)
		objectiveTrackerWatchdogElapsed = objectiveTrackerWatchdogElapsed + elapsed
		if objectiveTrackerWatchdogElapsed < 0.5 then return end
		objectiveTrackerWatchdogElapsed = 0

		if not (RQE.db and RQE.db.profile and RQE.db.profile.mythicScenarioMode) then return end

		local isInScenario = RQE.API and RQE.API.IsInScenario
			and RQE.API.IsInScenario()
			or (C_Scenario and C_Scenario.IsInScenario and C_Scenario.IsInScenario())

		if not isInScenario and RQE.UpdateTrackerVisibility then
	...
	function RQE:EnforceObjectiveTrackerVisibility()
		-- Classic Era's Edit Mode manages this Blizzard frame as protected. Changing
		-- it from an addon visibility hook can taint unrelated UI actions (such as
		-- the Game Menu Exit button), so leave it entirely to Blizzard on Classic.
...
--------------------------------------------------
	...
	-------------------------------------------------------
	-- #7e. Watched Quest Sorting & Distance Resolution
	-------------------------------------------------------

	-- Function that handles the sorting of tracked/watched quests based on proximity
	-- Classic uses RQE's database coordinates for same-zone proximity. This avoids
	-- running the full StartPeriodicChecks() step engine for every listed quest.
	function RQE:SortWatchedQuestsByProximity()
		...
		local playerX, playerY = playerPosition and playerPosition:GetXY()
		local sourceOrder = 0

		local function addMapID(mapIDs, location)
			local mapID = type(location) == "table" and tonumber(location.mapID)
			if mapID and mapID ~= 0 then mapIDs[mapID] = true end
		end

		local function getDatabaseMapIDs(questData)
			local mapIDs = {}
			if type(questData) ~= "table" then return mapIDs end

			addMapID(mapIDs, questData.location)
			addMapID(mapIDs, questData.coordinates)
			for _, location in ipairs(questData.locations or {}) do
				addMapID(mapIDs, location)
			end
			for _, stepData in ipairs(questData) do
				addMapID(mapIDs, stepData.coordinates)
				for _, hotspot in ipairs(stepData.coordinateHotspots or {}) do
					addMapID(mapIDs, hotspot)
				end
			end
			return mapIDs
		end

		local function getActiveStep(questID, questData)
			...
			-- A completed quest uses its explicit turn-in step.
			if isComplete or readyForTurnIn then
				for stepIndex, stepData in ipairs(questData) do
					if tonumber(stepData.objectiveIndex) == 99 then
						return stepIndex, stepData
					end
				end
			end

			-- Use the first database step whose Blizzard objective is incomplete.
			...
			-- Some DB steps are navigation-only. Use the first mapped step as a
			-- stable fallback when Blizzard does not expose a matching objective.
			for stepIndex, stepData in ipairs(questData) do
		...
		local function getStepDistanceSq(stepData)
			if not stepData or not playerMapID or not playerX or not playerY then return nil end
			local nearestDistanceSq
			local nearestLocation
			local function checkLocation(location)
				if type(location) ~= "table" or tonumber(location.mapID) ~= playerMapID then return end
				local x, y = tonumber(location.x), tonumber(location.y)
				if not x or not y then return end
				if x > 1 then x = x / 100 end
				if y > 1 then y = y / 100 end
				local distanceSq = (x - playerX) ^ 2 + (y - playerY) ^ 2
				if not nearestDistanceSq or distanceSq < nearestDistanceSq then
					nearestDistanceSq = distanceSq
					nearestLocation = location
				end
			end
			checkLocation(stepData.coordinates)
			for _, hotspot in ipairs(stepData.coordinateHotspots or {}) do
				checkLocation(hotspot)
			end
			return nearestDistanceSq, nearestLocation
		end

		-- Convert a same-map database coordinate to Blizzard world-space yards. This
		-- intentionally uses the location selected by getStepDistanceSq(), so the
		-- displayed number validates the same destination used for proximity sorting.
		local function getLocationDistanceYards(location)
			if type(location) ~= "table" or tonumber(location.mapID) ~= playerMapID then return nil end
			local x, y = tonumber(location.x), tonumber(location.y)
			...
			if C_Map.GetWorldPosFromMapPos and CreateVector2D then
				local _, playerWorldPosition = C_Map.GetWorldPosFromMapPos(playerMapID, CreateVector2D(playerX, playerY))
				local _, locationWorldPosition = C_Map.GetWorldPosFromMapPos(playerMapID, CreateVector2D(x, y))
				if playerWorldPosition and locationWorldPosition then
					local playerWorldX, playerWorldY = playerWorldPosition:GetXY()
					local locationWorldX, locationWorldY = locationWorldPosition:GetXY()
					if playerWorldX and playerWorldY and locationWorldX and locationWorldY then
						return math.sqrt((locationWorldX - playerWorldX) ^ 2 + (locationWorldY - playerWorldY) ^ 2)
					end
				end
			end

			-- HereBeDragons remains a fallback for map clients that do not provide
			-- usable C_Map world positions.
			if RQE.WPUtil and RQE.WPUtil.PlayerDistanceTo then
				local distance, unit = RQE.WPUtil.PlayerDistanceTo(playerMapID, x, y)
				if distance and unit == "yards" then return distance end
			end
		end

		-- Use the same database-coordinate and HereBeDragons distance path that
		-- supplies the working RQE Quest Helper "yards away" readout. The resulting
		-- coordinates are retained so the tracker labels can refresh while moving
		-- without rebuilding the whole tracker frame.
		local function getStepDisplayDistanceYards(questID, stepIndex)
			if not (stepIndex and RQE.GetDBStepCoordinates and RQE.WPUtil and RQE.WPUtil.PlayerDistanceTo) then return nil end
			local x, y, mapID = RQE:GetDBStepCoordinates(questID, stepIndex)
			if not (x and y and mapID) then return nil end
			local distance, unit = RQE.WPUtil.PlayerDistanceTo(mapID, x, y)
			if distance and unit == "yards" then
				return distance, mapID, x, y
			end
			return nil, mapID, x, y
		end

		-- Old active-step map test: this could classify a quest as local because
		-- one of its alternative hotspots was in the current zone, even when the
		-- selected waypoint was in another zone (for example, quest 109 -> Westfall).
		-- local function isActiveStepInPlayerZone(stepData)
		-- 	if not (stepData and playerMapID) then return false end
		-- 	local function isOnPlayerMap(location)
		-- 		return type(location) == "table" and tonumber(location.mapID) == playerMapID
		-- 	end
		-- 	if isOnPlayerMap(stepData.coordinates) then return true end
		-- 	for _, hotspot in ipairs(stepData.coordinateHotspots or {}) do
		-- 		if isOnPlayerMap(hotspot) then return true end
		-- 	end
		-- 	return false
		-- end

		-- The selected-destination map is intentionally not used for grouping. A
		-- database (or active Sandbox) quest with active-step coordinates is Primary
		-- whether its destination is local or in another zone.
		-- local function isSelectedDestinationInPlayerZone(destinationMapID)
		-- 	return playerMapID and tonumber(destinationMapID) == tonumber(playerMapID)
		-- end

		local function isNonDatabaseZoneQuest(questID)
			local questMapID
			if C_TaskQuest and type(C_TaskQuest.GetQuestZoneID) == "function" then
				questMapID = C_TaskQuest.GetQuestZoneID(questID)
			end
			if (not questMapID or questMapID == 0) and GetQuestUiMapID then
				questMapID = GetQuestUiMapID(questID)
			end
			return questMapID and questMapID == playerMapID
		end
		...
		local function getQuestLevel(questID)
			...
			-- Keep quests whose level is unavailable at the end of a level-sorted
			-- group while retaining their original quest-log order as a tie-breaker.
			return questLevel or math.huge
		end

		-- local function addQuestToRQETracker(questID, isVirtualWatch)
		-- 	if not questID or addedQuestIDs[questID] or RQE.API.IsWorldQuest(questID) then return end
		local function addQuestToRQETracker(questID, isVirtualWatch)
			-- Search results and virtual watches can retain a quest briefly after it
			-- has been turned in. Do not include a no-longer-live quest in the sorted
			-- list, because that list is also used for the Normal Quests header count.
			if not questID
				or addedQuestIDs[questID]
				or RQE.API.IsWorldQuest(questID)
				or not RQE.API.IsOnQuest(questID)
			then
				return
			end
			-- Virtual watches previously bypassed the hidden list here. Fresh manual
			-- and progress additions already clear their hidden flag, so honoring the
			-- flag during rendering preserves an explicit SC/ZQ untrack operation.
			-- if not isVirtualWatch
			-- 	and (RQE.ShowAllQuestLogQuests or RQE.ZoneQuestFilterMapID)
			-- 	and RQE.HiddenQuestLogQuests
			-- 	and RQE.HiddenQuestLogQuests[questID]
			-- then
			-- 	return
			-- end
			if (RQE.ShowAllQuestLogQuests or RQE.ZoneQuestFilterMapID)
				and RQE.HiddenQuestLogQuests
				and RQE.HiddenQuestLogQuests[questID]
			then
				return
			...
			local questData = RQE.getQuestData(questID)
			local hasDatabaseEntry = type(questData) == "table"
			local isZoneQuest = false
			local distanceSq = math.huge
			local questLevel = getQuestLevel(questID)
			local stepIndex, stepData, distanceLocation, distanceYards, distanceMapID, distanceX, distanceY
			local hasRQECoordinates = false

			if hasDatabaseEntry then
				stepIndex, stepData = getActiveStep(questID, questData)
				distanceYards, distanceMapID, distanceX, distanceY = getStepDisplayDistanceYards(questID, stepIndex)
				-- local mapIDs = getDatabaseMapIDs(questData)
				-- isZoneQuest = playerMapID and mapIDs[playerMapID] == true
				-- isZoneQuest = isActiveStepInPlayerZone(stepData)
				-- isZoneQuest = isSelectedDestinationInPlayerZone(distanceMapID)
				-- Any database/sandbox quest with active-step coordinates is Primary,
				-- even when those coordinates are on another map.  Its RQE distance is
				-- then compared globally with every other Primary quest.
				hasRQECoordinates = distanceMapID and distanceX and distanceY
				distanceSq, distanceLocation = getStepDistanceSq(stepData)
				distanceSq = distanceSq or math.huge
				-- distanceYards = getLocationDistanceYards(distanceLocation)
				-- Prefer the RQE Quest Helper's established PlayerDistanceTo path.
				distanceYards = distanceYards or getLocationDistanceYards(distanceLocation)
			else
				isZoneQuest = isNonDatabaseZoneQuest(questID)
			end

			-- Classic's native watch distance is a fallback only. The preferred value
			-- above remains the nearest active-step hotspot used by RQE's sorter.
			if not distanceYards and C_QuestLog.GetDistanceSqToQuest then
				local blizzardDistanceSq, onContinent = C_QuestLog.GetDistanceSqToQuest(questID)
				if blizzardDistanceSq and onContinent then
					distanceYards = math.sqrt(blizzardDistanceSq)
				end
			end

			-- Sort groups: DB zone quests by step distance, then non-DB zone quests
			-- by quest level, followed by all out-of-zone quests by quest level.
			-- local sortGroup = isZoneQuest and (hasDatabaseEntry and 1 or 2) or 3
			-- local sortGroup
			-- if hasDatabaseEntry and isZoneQuest and distanceYards then
			-- 	sortGroup = 1
			-- elseif isZoneQuest or (not hasDatabaseEntry and isNonDatabaseZoneQuest(questID)) then
			-- 	sortGroup = 2
			-- else
			-- 	sortGroup = 3
			-- end
			-- Primary: database/sandbox quests with active-step coordinates, regardless
			-- of their map. All other quests form the level-sorted fallback group.
			local sortGroup
			if hasDatabaseEntry and hasRQECoordinates then
				sortGroup = 1
			else
				sortGroup = 2
			end
		...
		if RQE.QuestTrackerSearchResults then
			-- A tracker search is a temporary, explicit virtual-watch source. It
			-- takes precedence over SC/ZQ and complete-quest display modes so only
			-- live player-log matches appear until Restore is pressed.
		...
		elseif RQE.ZoneQuestFilterMapID then
			-- ZQ is also RQE-only: use the database-derived zone list instead of
			-- changing Blizzard's watched-quest list.
			for _, questID in ipairs(RQE.ZoneQuests[RQE.ZoneQuestFilterMapID] or {}) do
				addQuestToRQETracker(questID)
			end
		elseif RQE.ShowAllQuestLogQuests then
			-- SC is an RQE-only display mode: enumerate the player's quest log without
			-- adding any Blizzard quest watches.
			local numEntries = RQE.API.GetNumQuestLogEntries()
			for questLogIndex = 1, numEntries do
				local info = RQE.API.GetQuestLogInfo(questLogIndex)
				if info and not info.isHeader then
					addQuestToRQETracker(info.questID)
				end
			end
		else
			local numTrackedQuests = RQE.API.GetNumQuestWatches()
			for watchIndex = 1, numTrackedQuests do
				addQuestToRQETracker(RQE.API.GetQuestIDForQuestWatchIndex(watchIndex))
			end
		end

		-- Normal/SC mode retains all RQE virtual watches. ZQ deliberately retains
		-- only virtual watches created by an explicit Blizzard Quest Log selection;
		-- its automatic source remains RQE.ZoneQuests[currentMap]. This prevents an
		...
		elseif RQE.ShowOnlyAllCompleteVirtualQuests then
			virtualWatchSource = {}
		elseif RQE.ZoneQuestFilterMapID then
			virtualWatchSource = RQE.ClassicManualVirtualQuestWatches or {}
		end
		for questID in pairs(virtualWatchSource) do
			if RQE.API.IsOnQuest(questID) then
				addQuestToRQETracker(questID, true)
			end
		end

		-- Sort DB-backed zone quests by their current incomplete objective's nearest
		-- same-map hotspot. Other groups retain their original quest-list order.
		-- Sort DB-backed zone quests by their current incomplete objective's nearest
		-- same-map hotspot. Non-DB in-zone quests and all out-of-zone quests sort
		-- by ascending quest level, then preserve source order for equal levels.
		-- table.sort(RQE.SortedWatchedQuests, function(a, b)
			-- if a.sortGroup ~= b.sortGroup then return a.sortGroup < b.sortGroup end
			-- if a.sortGroup == 1 and a.distanceSq ~= b.distanceSq then
				-- return a.distanceSq < b.distanceSq
			-- end
			-- if (a.sortGroup == 2 or a.sortGroup == 3) and a.questLevel ~= b.questLevel then
				-- return a.questLevel < b.questLevel
			-- end
			-- return a.sourceOrder < b.sourceOrder
		-- end)

		-- Primary entries use the same yard distance shown in the tracker. Secondary
		-- and Tertiary entries fall back to quest level, then their original order.
		table.sort(RQE.SortedWatchedQuests, function(a, b)
			if a.sortGroup ~= b.sortGroup then return a.sortGroup < b.sortGroup end
			if a.sortGroup == 1 then
				-- Keep coordinate-bearing DB/Sandbox quests Primary even if a distance
				-- provider is briefly unavailable; those entries sort after measured
				-- distances instead of causing a nil comparison error.
				local aDistance = a.distanceYards or math.huge
				local bDistance = b.distanceYards or math.huge
		...
		if RQE.db.profile.debugLevel == "INFO+" then
			...
			for i, questData in ipairs(RQE.SortedWatchedQuests) do
				local displayDistance = questData.distanceSq < math.huge and math.sqrt(questData.distanceSq) or "N/A"
				print(i .. ". QuestID:", questData.questID, "- Group:", questData.sortGroup, "- Level:", tostring(questData.questLevel), "- Step:", tostring(questData.stepIndex), "- Distance:", displayDistance)
			end
		end
	end
			...
			if questButton and questButton.QuestZoneInfo and questButton.rqeQuestZoneText
				...
				for _, trackedQuest in ipairs(RQE.SortedWatchedQuests) do
					if trackedQuest.questID == questButton.questID then
						distanceYards = trackedQuest.distanceYards
						break
					end
				end
				local distanceText = distanceYards and string.format(" | %.0f yds", distanceYards) or " | N/A"
				questButton.QuestZoneInfo:SetText(questButton.rqeQuestZoneText .. distanceText)
			end
		end
	end


	-- Rebuild the tracker only when movement has made a different Primary quest
	-- meaningfully nearer. Distance labels refresh separately at a higher rate;
	-- this keeps the more expensive frame rebuild to at most once per second.
	function RQE:MaybeRefreshTrackerOrderWhileMoving()
		local now = GetTime()
		if RQE.LastMovementTrackerSortTime and (now - RQE.LastMovementTrackerSortTime) < 1 then return end
		RQE.LastMovementTrackerSortTime = now

		local trackedQuests = RQE.SortedWatchedQuests
		if not trackedQuests or #trackedQuests == 0 or InCombatLockdown() then return end

		local displayedFirstQuest = trackedQuests[1]
		local nearestPrimaryQuest
		for _, trackedQuest in ipairs(trackedQuests) do
			if trackedQuest.sortGroup == 1 and trackedQuest.distanceYards then
				if not nearestPrimaryQuest or trackedQuest.distanceYards < nearestPrimaryQuest.distanceYards then
					nearestPrimaryQuest = trackedQuest
				end
			end
		end

		if not nearestPrimaryQuest or nearestPrimaryQuest == displayedFirstQuest then return end

		-- Hold the existing top row until the new nearest quest is at least 25
		-- yards closer. This prevents two nearly tied quests from constantly
		-- swapping positions while the player moves.
		local displayedDistance = displayedFirstQuest and displayedFirstQuest.distanceYards
		local shouldReorder = not displayedFirstQuest
			or displayedFirstQuest.sortGroup ~= 1
			or not displayedDistance
			or nearestPrimaryQuest.distanceYards + 25 < displayedDistance

		if shouldReorder and UpdateRQEQuestFrame then
			UpdateRQEQuestFrame()
		end
	end


	-- A stop event is infrequent, so always perform one final accurate rebuild.
	function RQE:RefreshTrackerOrderAfterMovement()
		if InCombatLockdown() then return end
		RQE.LastMovementTrackerSortTime = GetTime()
	...
	function ShowQuestDropdown(self, questID)
		MenuUtil.CreateContextMenu(UIParent, function(ownerRegion, rootDescription)
			...
			-- rootDescription:CreateButton("Untrack Quest", function() C_QuestLog.RemoveQuestWatch(questID); RQE:RemoveClassicVirtualQuestWatch(questID); RQE:ClearRQEQuestFrame(); UpdateRQEQuestFrame() end)
			rootDescription:CreateButton("Untrack Quest", function()
				-- SC/ZQ source quests independently of Blizzard's watch list, so retain
				-- an explicit RQE hide when this was a virtual-only quest. The shared
				-- helper also removes an existing Blizzard checkmark and RQE's virtual
				-- mirror, if present.
			...
			local questLineInfo = C_QuestLine and C_QuestLine.GetQuestLineInfo and C_QuestLine.GetQuestLineInfo(questID, uiMapID)
...
--------------------------------------------------
	...
	-- TBC's timer API reports only the currently selected quest, so countdowns
	...
	-- Classic uses its virtual-watch list for these results. The matching itself
	-- still comes from the live quest log, with RQEDatabase text as an additional
	-- source when a database entry exists.
	-------------------------------------------------------
	-- #11b. Search Matching & Classic Watch Capture
	-------------------------------------------------------
	...
	local function IsQuestTrackerSearchableQuestID(questID)
		local questLogIndex = questID and RQE.API.GetLogIndexForQuestID(questID)
		return questLogIndex and IsQuestTrackerSearchableLogQuest(questID, RQE.API.GetQuestLogInfo(questLogIndex))
	end
	...
	local function FindQuestTrackerSearchMatches(searchTerm)
		...
		for questLogIndex = 1, numEntries do
			...
			if IsQuestTrackerSearchableLogQuest(questID, info) then
				...
				if RQE.API.GetQuestLogText then
					local descriptionText, objectivesText = RQE.API.GetQuestLogText(questLogIndex)
	...
	local function CopyQuestTrackerSearchSet(source)
		local copy = {}
		for questID, value in pairs(source or {}) do
			if value then
				copy[questID] = value
			end
		end
	...
	local function CollectClassicQuestTrackerNativeWatches()
		...
		for watchIndex = 1, numWatches do
			...
			if IsQuestTrackerSearchableQuestID(questID) then
	...
	local function RemoveClassicQuestTrackerSearchWatches()
		for _, questID in ipairs(CollectClassicQuestTrackerNativeWatches()) do
			if RQE.API.RemoveQuestWatch then
				RQE.API.RemoveQuestWatch(questID)
			end
		end

		for questID in pairs(RQE.ClassicVirtualQuestWatches or {}) do
			if IsQuestTrackerSearchableQuestID(questID) then
				RQE.ClassicVirtualQuestWatches[questID] = nil
			end
		end
		for questID in pairs(RQE.ClassicManualVirtualQuestWatches or {}) do
			if IsQuestTrackerSearchableQuestID(questID) then
				RQE.ClassicManualVirtualQuestWatches[questID] = nil
			end
		end
		for questID in pairs(RQE.AutoQuestProgressVirtualWatches or {}) do
			if IsQuestTrackerSearchableQuestID(questID) then
				RQE.AutoQuestProgressVirtualWatches[questID] = nil
			end
		end
		for questID in pairs(RQE.AutoQuestProgressWatchTokens or {}) do
			if IsQuestTrackerSearchableQuestID(questID) then
				RQE.AutoQuestProgressWatchTokens[questID] = nil
			end
		end
	end
	...
	-- A quest-tracker search temporarily owns the tracker display. When its final
	-- matched quest leaves the quest log, restore the tracker state that existed
	-- before the search so new quests can be displayed normally.
	function RQE:RemoveInactiveQuestTrackerSearchResult(questID)
		if not questID
			or not self.QuestTrackerSearchResults
			or not self.QuestTrackerSearchResults[questID]
			or RQE.API.IsOnQuest(questID)
		then
			return false
		end

		self.QuestTrackerSearchResults[questID] = nil
		if next(self.QuestTrackerSearchResults) == nil then
			self:RestoreQuestTrackerSearch()
		else
	...
	function RQE:SearchQuestTracker(searchText)
		...
		if not self.QuestTrackerSearchRestoreState then
			self.QuestTrackerSearchRestoreState = {
				nativeWatches = CollectClassicQuestTrackerNativeWatches(),
				classicVirtualQuestWatches = CopyQuestTrackerSearchSet(self.ClassicVirtualQuestWatches),
				classicManualVirtualQuestWatches = CopyQuestTrackerSearchSet(self.ClassicManualVirtualQuestWatches),
				autoQuestProgressVirtualWatches = CopyQuestTrackerSearchSet(self.AutoQuestProgressVirtualWatches),
				autoQuestProgressWatchTokens = CopyQuestTrackerSearchSet(self.AutoQuestProgressWatchTokens),
				hiddenQuestLogQuests = CopyQuestTrackerSearchSet(self.HiddenQuestLogQuests),
			}
		end

		RemoveClassicQuestTrackerSearchWatches()
		...
		self.HiddenQuestLogQuests = self.HiddenQuestLogQuests or {}
	...
	function RQE:RestoreQuestTrackerSearch()
		...
		RemoveClassicQuestTrackerSearchWatches()
		self.ClassicVirtualQuestWatches = CopyQuestTrackerSearchSet(restoreState.classicVirtualQuestWatches)
		self.ClassicManualVirtualQuestWatches = CopyQuestTrackerSearchSet(restoreState.classicManualVirtualQuestWatches)
		self.AutoQuestProgressVirtualWatches = CopyQuestTrackerSearchSet(restoreState.autoQuestProgressVirtualWatches)
		self.AutoQuestProgressWatchTokens = CopyQuestTrackerSearchSet(restoreState.autoQuestProgressWatchTokens)
		self.HiddenQuestLogQuests = CopyQuestTrackerSearchSet(restoreState.hiddenQuestLogQuests)

		for _, questID in ipairs(restoreState.nativeWatches or {}) do
	...
	local function colorizeObjectives(questID)
		...
		local isEscortObjectiveComplete = RQE.ClassicEscortCompletedQuests and RQE.ClassicEscortCompletedQuests[questID]
	...
	function RQE:QuestRewardsTooltip(tooltip, questID)
		...
		-- Pre-retail quest reward APIs read the selected legacy quest-log entry and
		-- do not provide C_QuestLog.GetSelectedQuest/SetSelectedQuest.
		if RQE.API.GameVersion and RQE.API.GameVersion.usesLegacyQuestLogSelection then
			local previousQuestID = C_QuestLog.GetSelectedQuest and C_QuestLog.GetSelectedQuest()
			local previousQuestLogIndex = type(GetQuestLogSelection) == "function" and GetQuestLogSelection() or nil
			local questLogIndex = RQE.API.GetLogIndexForQuestID(questID)
			-- if C_QuestLog.SetSelectedQuest then
			-- 	C_QuestLog.SetSelectedQuest(questID)
			-- end

			-- Classic's choice-reward APIs read the legacy selected quest-log entry
			-- and ignore their questID argument. Select the hovered quest by its log
			-- index before reading every reward so tracker rows cannot borrow choices
			-- from whichever quest happens to be selected in the Blizzard quest log.
			...
			-- local rewardMoney = GetQuestLogRewardMoney(questID) or 0
			-- local rewardItemsCount = GetNumQuestLogRewards(questID) or 0
			-- local choiceItemsCount = GetNumQuestLogChoices(questID) or 0
			local rewardXP = GetQuestLogRewardXP() or 0
			local rewardMoney = GetQuestLogRewardMoney() or 0
			local rewardItemsCount = GetNumQuestLogRewards() or 0
			local choiceItemsCount = GetNumQuestLogChoices() or 0
			local hasRewards = rewardXP > 0 or rewardMoney > 0 or rewardItemsCount > 0 or choiceItemsCount > 0

			if hasRewards then
				tooltip:AddLine("Rewards:", 1, 1, 1)

				if rewardXP > 0 then
					local formattedXP = FormatLargeNumber and FormatLargeNumber(rewardXP) or tostring(rewardXP)
					tooltip:AddLine("Experience: " .. formattedXP .. " XP", 1, 1, 1)
				end

				if rewardMoney > 0 then
					tooltip:AddLine("Money: " .. GetCoinTextureString(rewardMoney), 1, 1, 1)
				end

				if rewardItemsCount > 0 then
					tooltip:AddLine("You will receive:", 1, 1, 1)
					for i = 1, rewardItemsCount do
						-- local itemName, texture, numItems, quality = GetQuestLogRewardInfo(i, questID)
						local itemName, texture, numItems, quality = GetQuestLogRewardInfo(i)
						if itemName then
							local quantity = (numItems and numItems > 1) and (numItems .. "x ") or ""
							local itemText = (texture and "|T" .. texture .. ":16|t " or "") .. quantity .. itemName
							local color = ITEM_QUALITY_COLORS[quality] or { r = 1, g = 1, b = 1 }
							tooltip:AddLine(itemText, color.r, color.g, color.b)
						end
					end
				end

				if choiceItemsCount > 0 then
					tooltip:AddLine(choiceItemsCount == 1 and "You may choose:" or "Choose one of the following:", 1, 1, 1)
					for i = 1, choiceItemsCount do
						-- local itemName, texture, numItems, quality = GetQuestLogChoiceInfo(i, questID)
						local itemName, texture, numItems, quality = GetQuestLogChoiceInfo(i)
						if itemName then
							local quantity = (numItems and numItems > 1) and (numItems .. "x ") or ""
							local itemText = (texture and "|T" .. texture .. ":16|t " or "") .. quantity .. itemName
							local color = ITEM_QUALITY_COLORS[quality] or { r = 1, g = 1, b = 1 }
			...
			if previousQuestLogIndex and type(SelectQuestLogEntry) == "function" then
				SelectQuestLogEntry(previousQuestLogIndex)
			elseif previousQuestID then
				local previousQuestIDLogIndex = RQE.API.GetLogIndexForQuestID(previousQuestID)
				if previousQuestIDLogIndex and type(SelectQuestLogEntry) == "function" then
					SelectQuestLogEntry(previousQuestIDLogIndex)
				elseif C_QuestLog.SetSelectedQuest then
	...
	function RQE:QuestType()
		local numTrackedQuests = RQE.API.GetNumQuestWatches()
		local numTrackedWorldQuests = RQE.API.GetNumWorldQuestWatches()
	...
	function UpdateRQEQuestFrame()
		...
		local supportsAchievements = RQE.API.GameVersion and RQE.API.GameVersion.supportsAchievements
		...
		local numTrackedQuests = #RQE.SortedWatchedQuests
		...
		for i, questData in ipairs(RQE.SortedWatchedQuests) do
			...
			local questIndex = RQE.API.GetLogIndexForQuestID(questID)
				...
				if info and not info.isHeader then
					...
					-- QuestLogIndexButton:SetScript("OnClick", function(self, button)	-- changed as was causing issues when Mythic/Scenario mode was enabled the button wouldn't click
						...
						if shiftLeftClick then
							...
							-- Untrack the quest
							if RQE.ShowAllQuestLogQuests or RQE.ZoneQuestFilterMapID then
								-- SC and ZQ source entries from RQE's own lists rather than
								-- Blizzard's watch list. Preserve the same-zone RQE hide, and
								-- also remove both a native checkmark and virtual mirror.
								-- RQE.HiddenQuestLogQuests = RQE.HiddenQuestLogQuests or {}
								-- RQE.HiddenQuestLogQuests[questID] = true
								RQE:UntrackQuestEverywhere(questID)

								local displayedQuestID = RQE.DisplayedQuestID
								if questID == displayedQuestID then
									RQE:ClearFrameData()
							...
							-- A real QuestLogIndexButton click reaches this handler directly,
							...
							if InCombatLockdown() then
								...
								RQE.DisplayedQuestID = questID
								RQE:SaveSuperTrackedQuestToCharacter()
								RQE:UpdateRQEFrameCombatSafe(questID)
							...
							-- Refresh the UI here to update the button state
							UpdateRQEQuestFrame()
							-- The selected quest's waypoint/active step is established
							-- asynchronously. Refresh the tracker yard labels shortly after
					...
					local questObjectivesText = RQE.API.GetQuestLogObjectivesText(questIndex)
					...
					-- 	local _, _, classicQuestTag = GetQuestLogTitle(questIndex)
					-- 	questTag = classicQuestTag
					-- end

					-- GetQuestLogTitle's third return value is the suggested group size,
					-- not the quest tag. Read the explicit Classic tag for Dungeon, Elite,
					-- and PvP quest labels instead.
					local questTag
					...
					-- Classic's quest-log API can return the suggested group size as a
					-- numeric string. Normalize it before comparing it below.
					suggestedSize = tonumber(info.suggestedGroup) or 0
					...
					QuestLogIndexButton.rqeQuestLevel = questLevel
					...
					if (not questZoneMapID or questZoneMapID == 0) and C_TaskQuest and C_TaskQuest.GetQuestZoneID then
						questZoneMapID = C_TaskQuest.GetQuestZoneID(questID)
					end
					local questZoneMapInfo = questZoneMapID and C_Map and C_Map.GetMapInfo and C_Map.GetMapInfo(questZoneMapID)
					local questZoneText = (questZoneMapID and questZoneMapID > 0)
						and ("#" .. questZoneMapID .. " - " .. (questZoneMapInfo and questZoneMapInfo.name or "Unknown zone"))
						or "#? - Zone unavailable"
					-- local questDistanceText = "| N/A"
					-- if questData.distanceLocation and RQE.WPUtil and RQE.WPUtil.PlayerDistanceTo then
						-- local distance, unit = RQE.WPUtil.PlayerDistanceTo(
							-- questData.distanceLocation.mapID,
							-- questData.distanceLocation.x,
							-- questData.distanceLocation.y
						-- )
						-- if distance and unit == "yards" then
							-- questDistanceText = string.format("| %.0f yds", distance)
						-- end
					-- end
					-- Use the sorting function's stored yard value, derived from Blizzard's
					-- world-space map positions rather than HereBeDragons' map cache.
					local questDistanceText = questData.distanceYards and string.format(" | %.0f yds", questData.distanceYards) or " | N/A"

					-- Create or reuse the zone label immediately below the level/name label.
					local QuestZoneInfo = RQE.QuestLogIndexButtons[i].QuestZoneInfo or QuestLogIndexButton:CreateFontString(nil, "OVERLAY", "GameFontNormal")
					QuestZoneInfo:SetPoint("TOPLEFT", QuestLevelAndName, "BOTTOMLEFT", 0, -3)
					QuestZoneInfo:SetFont("Fonts\\FRIZQT__.TTF", 11, "OUTLINE")
					-- QuestZoneInfo:SetTextColor(1, 1, 1)
					QuestZoneInfo:SetTextColor(0, 212/255, 212/255) -- Bright teal: #00D4D4
					QuestZoneInfo:SetJustifyH("LEFT")
					QuestZoneInfo:SetJustifyV("TOP")
					QuestZoneInfo:SetWidth(RQE.RQEQuestFrame:GetWidth() - 110)
					QuestZoneInfo:SetHeight(0)
					-- QuestZoneInfo:SetText(questZoneText)
					QuestZoneInfo:SetText(questZoneText .. questDistanceText)
					QuestZoneInfo:Show()
					...
					-- QuestDistanceInfo:SetPoint("TOPRIGHT", QuestLevelAndName, "BOTTOMRIGHT", 0, -3)
					-- QuestDistanceInfo:SetFont("Fonts\\FRIZQT__.TTF", 11, "OUTLINE")
					-- QuestDistanceInfo:SetTextColor(1, 1, 1)
					-- QuestDistanceInfo:SetJustifyH("RIGHT")
					-- QuestDistanceInfo:SetJustifyV("TOP")
					-- QuestDistanceInfo:SetText(questDistanceText)
					-- QuestDistanceInfo:Show()
					-- QuestLogIndexButton.QuestDistanceInfo = QuestDistanceInfo
					if QuestLogIndexButton.QuestDistanceInfo then
						QuestLogIndexButton.QuestDistanceInfo:Hide()
					end
					...
					if RQE.db.profile.enableQuestTypeDisplay then
						...
						QuestTypeLabel:SetPoint("TOPLEFT", QuestZoneInfo, "BOTTOMLEFT", 0, -3)
					...
					if RQE.db.profile.enableQuestTypeDisplay then
						...
						QuestObjectivesOrDescription:SetPoint("TOPLEFT", QuestZoneInfo, "BOTTOMLEFT", 0, -3)
					end
					...
					QuestLevelAndName:SetScript("OnEnter", function(self)
						-- Explicitly clear the shared tooltip so rewards from the previously
						-- hovered tracked quest cannot carry over to this quest.
		...
		RQE:UpdateTrackedQuestLevelColors()
		...
		if not supportsAchievements and RQE.AchievementsFrame then
			RQE.AchievementsFrame:Hide()
		end
		...
		for i, questInfo in ipairs(sortedWorldQuests) do
			...
			if questID then
				...
				WQuestLevelAndName:SetScript("OnEnter", function(self)
					...
					-- Add objectives
					local objectivesText = RQE.API.GetQuestLogObjectivesText(C_QuestLog.GetLogIndexForQuestID(questID))
					if objectivesText and objectivesText ~= "" then
						GameTooltip:AddLine("Objectives:")
	...
	function RQE.ClickRandomQuestLogIndexButton(bigQuestID)
		...
		RQE.API.SetSuperTrackedQuestID(randomQuestID)
	...
	function RQE:CreateOrUpdateQuestItemButton(questID, questLogIndex, parent)
		if InCombatLockdown() then
			RQE.debugLog("Skipping secure quest-item button update during combat.")
			return
		end
	...
	function RQE:ClickWorldQuestButton(questID)
		if InCombatLockdown() then
			RQE.WorldQuestButtonToClickAfterCombat = questID
			return
		end
	...
	function RQE.UpdateTrackedAchievementList()
		...
		if not (RQE.API.GameVersion and RQE.API.GameVersion.supportsAchievements) then
			RQE.TrackedAchievementIDs = {}
			if RQE.AchievementsFrame then
				RQE.AchievementsFrame.achieveCount = 0
				RQE.AchievementsFrame:Hide()
			end
		...
		if not (C_ContentTracking and C_ContentTracking.GetTrackedIDs
			and Enum and Enum.ContentTrackingType and Enum.ContentTrackingType.Achievement)
		then
			RQE.TrackedAchievementIDs = {}
			if RQE.AchievementsFrame then
				RQE.AchievementsFrame.achieveCount = 0
		...
		local achievementIDs = C_ContentTracking.GetTrackedIDs(Enum.ContentTrackingType.Achievement) or {}
		RQE.TrackedAchievementIDs = achievementIDs
		UpdateRQEAchievementsFrame()
	end
	...
	function RQE:CheckWatchedQuestsSync()
		...
		for i = 1, RQE.API.GetNumQuestWatches() do
			local qID = RQE.API.GetQuestIDForQuestWatchIndex(i)
			if qID then
		...
		for i = 1, RQE.API.GetNumWorldQuestWatches() do
			local qID = RQE.API.GetQuestIDForWorldQuestWatchIndex(i)
			if qID then
				watchedQuests[qID] = true
			end
		end
	...
	if not (RQE.API and not RQE.API.CanManageBlizzardObjectiveTracker()) then
		C_Timer.NewTicker(1, function()
		if InCombatLockdown() then return end
