...
WaypointManager.lua
TBC waypoint routing, provider compatibility, map pins, and quest-aware coordinate resolution

]]


--------------------------------------------------
-- #1. 📦 Namespace & Provider Compatibility
--------------------------------------------------

	-------------------------------------------------------
	-- #1a. Addon Namespace & Provider Globals
	-------------------------------------------------------

	RQE = RQE or {}  -- Initialize the RQE table if it's not already initialized
	RQE.Frame = RQE.Frame or {}

	-- Keep references local.  Initializing another addon's global table from RQE
	-- can taint that addon's UI when it is later loaded.
	---@type any
...
--------------------------------------------------
-- #2. 🚧 Waypoint Policy & Coordinate Validation
--------------------------------------------------

	-------------------------------------------------------
	-- #2a. Quest Exclusion Registry
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #2b. Manager State Initialization
	-------------------------------------------------------
...
--------------------------------------------------
-- #3. 📍 Direct Waypoint Creation & Provider Dispatch
--------------------------------------------------

	-------------------------------------------------------
	-- #3a. Low-Level Waypoint Creation
	-------------------------------------------------------
...
--------------------------------------------------
-- #4. 🧭 Quest-Aware Waypoint Resolution
--------------------------------------------------

	-------------------------------------------------------
	-- #4a. Unknown Quest Route Dispatcher
	-------------------------------------------------------
	...
	function RQE:CreateUnknownQuestWaypoint(questID, mapID)
		...
		C_Timer.After(0.5, function()
			...
			local waypointText = RQE.API.GetNextWaypointText(questID)
	...
	function RQE:CreateSearchedQuestWaypoint(questID, mapID)
		...
		local isComplete = RQE.API.IsQuestFlaggedCompleted(questID)
		local isInLog = RQE.API.GetLogIndexForQuestID(questID)
	...
	-------------------------------------------------------
	-- #4c. Searched Quest Step Waypoints
	-------------------------------------------------------


	-- Create a waypoint for a searched quest's virtual pickup step (0) or one of
	-- its DB steps. Searched quests are not Blizzard-supertracked in Classic, so
	-- they cannot use the normal supertrack-only step coordinate resolver.
	function RQE:CreateSearchedQuestStepWaypoint(questID, stepIndex)
		local questData = RQE.getQuestData(questID)
		if not questData then return end

		if stepIndex == 0 then
			return RQE:CreateSearchedQuestWaypoint(questID, C_Map.GetBestMapForUnit("player"))
		end
		...
		local mapID, x, y
		if RQE.WPUtil and RQE.WPUtil.SelectBestHotspot and stepData.coordinateHotspots then
			mapID, x, y = RQE.WPUtil.SelectBestHotspot(questID, stepIndex, stepData)
		end
		if not (mapID and x and y) and stepData.coordinates then
			mapID = stepData.coordinates.mapID
			x = stepData.coordinates.x
			y = stepData.coordinates.y
			if x and x > 1 then x = x / 100 end
			if y and y > 1 then y = y / 100 end
		end
	...
	-------------------------------------------------------
	-- #4d. Direction-Text Waypoint Resolution
	-------------------------------------------------------
	...
	function RQE:CreateUnknownQuestWaypointWithDirectionText(questID, mapID)
		...
		local qType = RQE.API.GetQuestType(questID)
		...
		-- end
		-- Carbonite compatibility now synchronizes the selected QuestLogIndexButton
		-- quest through RQE:SyncCarboniteQuestTracking(), rather than adding pins here.
	...
	-------------------------------------------------------
	-- #4e. World Quest Replacement Guard
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #4f. Coordinate Fallback Without Direction Text
	-------------------------------------------------------
	...
	function RQE:CreateUnknownQuestWaypointNoDirectionText(questID, mapID)
		...
		if questData and not RQE.API.IsOnQuest(questID) and questData.location then
			...
			C_Timer.After(0.1, function()
				...
				local questType = RQE.API.GetQuestType(questID)
...
--------------------------------------------------
-- #5. 🗺️ Step & Blizzard Waypoint Routes
--------------------------------------------------

	-------------------------------------------------------
	-- #5a. Database Step Waypoints
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5b. Blizzard Next-Waypoint Route
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5c. Current-Map Super-Tracked Route
	-------------------------------------------------------
...
--------------------------------------------------
-- #6. 🧹 Waypoint Removal & Map Pins
--------------------------------------------------

	-------------------------------------------------------
	-- #6a. Stored Waypoint Removal
	-------------------------------------------------------
...
--------------------------------------------------
-- #7. 🖱️ Coordinate Interaction & Forced Refresh
--------------------------------------------------

	-------------------------------------------------------
	-- #7a. Coordinate Click Handling
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #7b. Forced Super-Tracked Quest Routing
	-------------------------------------------------------
...
--------------------------------------------------
-- #8. 🏷️ Waypoint Title Resolution
--------------------------------------------------

	-------------------------------------------------------
	-- #8a. Centralized Quest & Hotspot Titles
	-------------------------------------------------------
...
--------------------------------------------------
-- #9. ✅ Finalization & Export Notes
--------------------------------------------------

	-------------------------------------------------------
	-- #9a. Cross-File Export Boundary
	-------------------------------------------------------
