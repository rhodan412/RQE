...
DebugLog.lua
Debug message capture, filtering, display, export, and legacy print integration

]]


--------------------------------------------------
-- #1. 📦 Namespace, Log State & Capture Parsing
--------------------------------------------------

	-------------------------------------------------------
	-- #1a. Addon Namespace & Client Mode
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #1d. Capture State Reset
	-------------------------------------------------------

	-- Function to reset all temporary filtering and automatic capture state
	...
	-------------------------------------------------------
	-- #1e. Contribution Payload Recognition
	-------------------------------------------------------

	-- Function to extract the quest ID from a formatted missing-text capture header
	...
	-- Function to extract the quest ID from an NPC contribution capture header
...
--------------------------------------------------
-- #2. 📥 Log Ingestion & Chat Interception
--------------------------------------------------

	-------------------------------------------------------
	-- #2a. Filtered Log Entry Ingestion
	-------------------------------------------------------

	-- Function to add messages to the log
	function RQE.AddToDebugLog(message)
		-- Check if debug logging is enabled via the checkbox
		-- Capture is a temporary session scope, never a saved preference change.
		local capturing = captureMode == "all" or capturePhase ~= nil
		if not RQE or not RQE.db or not RQE.db.profile
			or (not RQE.db.profile.debugLoggingCheckbox and not capturing) then
			return
		end
		...
		if RQE.db.profile.debugTimeStampCheckbox and not capturing then
...
--------------------------------------------------
-- #3. 🖥️ Debug Log Window Construction
--------------------------------------------------

	-------------------------------------------------------
	-- #3a. Display Entry Points
	-------------------------------------------------------
	...
	-- Function to show the Debug Log through the legacy display entry point
	...
	-------------------------------------------------------
	-- #3d. Scroll Container & Editable Log Text
	-------------------------------------------------------
...
--------------------------------------------------
-- #4. 🔄 Frame Refresh & Window Controls
--------------------------------------------------

	-------------------------------------------------------
	-- #4a. Text Measurement & Frame Refresh
	-------------------------------------------------------
...
--------------------------------------------------
-- #5. 🎛️ Visibility, Commands & Scrolling
--------------------------------------------------

	-------------------------------------------------------
	-- #5a. Log Toggle Slash Command
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5b. Printed Output Formatting
	-------------------------------------------------------

	-- Function to safely combine printed arguments into one Debug Log message
	...
	-------------------------------------------------------
	-- #5d. Scroll Position & Text-Change Handlers
	-------------------------------------------------------
...
--------------------------------------------------
-- #6. 🧹 Log Clearing & Explicit Export Capture
--------------------------------------------------

	-------------------------------------------------------
	-- #6a. Log Content & Session Clearing
	-------------------------------------------------------

	-- Function to empty the stored log and return its display to the top
	...
	-------------------------------------------------------
	-- #6b. Full Export Capture Lifecycle
	-------------------------------------------------------
	...
	-- Function to end a full export capture while preserving its collected output
...
--------------------------------------------------
-- #7. 📜 Automatic Quest Data Capture
--------------------------------------------------

	-------------------------------------------------------
	-- #7a. Session Status & Quest Registration
	-------------------------------------------------------

	-- Function to return the token used to reject stale delayed capture work
	function RQE:GetDebugLogCaptureGeneration()
		return captureGeneration
	end


	-- Function to report whether automatic quest-data capture is currently active
	...
	-------------------------------------------------------
	-- #7c. Text & NPC Capture Phases
	-------------------------------------------------------

	-- Function to begin collecting missing text output for a registered quest
	...
	-- Function to begin collecting missing NPC output for a registered quest
	...
	-- Function to close the current text or NPC phase without ending the shared session
...
--------------------------------------------------
-- #8. 💬 Legacy Print Capture Bridge
--------------------------------------------------

	-------------------------------------------------------
	-- #8a. Producer Output Capture
	-------------------------------------------------------
	...
	function RQE:CapturePrintedDebugOutput(producer, ...)
		...
		-- Temporarily mirror legacy print output into the Debug Log while the producer runs
		_G.print = function(...)
			originalPrint(...)
