...
WaypointManager.lua
Waypoint creation, quest-route resolution, provider integration, and map-pin management
...
--------------------------------------------------
	...
	-------------------------------------------------------
	-- #4c. Direction-Text Waypoint Resolution
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #4d. World Quest Replacement Guard
	-------------------------------------------------------
	...
	function RQE:ShouldBlockWorldQuestWaypoint(candidateQuestID)
		...
		local intendedQuestID = RQE.DisplayedQuestID or RQE.API.GetSuperTrackedQuestID()
	...
	-------------------------------------------------------
	-- #4e. Coordinate Fallback Without Direction Text
	-------------------------------------------------------
	...
	function RQE:CreateUnknownQuestWaypointNoDirectionText(questID, mapID)
		...
		if questData and not RQE.API.IsOnQuest(questID) and questData.location then
			...
			C_Timer.After(0.1, function()
				...
				local questID = extractedQuestID or RQE.API.GetSuperTrackedQuestID()
				...
				if not questID then
					questID = RQE.API.GetSuperTrackedQuestID() or 0	 or 0  -- Fallback to super-tracked quest or set to 0
				end
				...
				local waypointTitle = RQE:GetWaypointTitle(questID, mapID, (x and x/100) or nil, (y and y/100) or nil)
...
--------------------------------------------------
	...
	function RQE:CreateSuperTrackedQuestWaypointFromNextWaypointOnCurrentMap()
		...
		local questID = RQE.API.GetSuperTrackedQuestID() or extractedQuestID or extractedQuestID
		if not questID or questID == 0 then
			if RQE.db.profile.debugLevel == "INFO+" then
