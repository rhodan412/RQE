...
function RQEMacro:SetQuestStepMacro(questID, stepIndex, macroContent, perCharacter)
	...
	if InCombatLockdown() then
		...
		-- Updating tooltip scripts on the secure Magic Button is not safe during
		-- combat.  UPDATE_MACROS will refresh it after the queued macro work is
...
-- Writes the virtual step 0 pickup macro for a searched quest. This helper is
-- deliberately not gated on quest-log state so returning from step 1 can
-- always replace that step's macro with the quest-giver macro.
function RQE:SetSearchedQuestPickupMacro(questID)
	...
	-- A searched quest that is not yet in the log is RQE's virtual pickup step.
...
function RQEMacro:ActuallySetMacro(name, iconFileID, body, perCharacter)
	...
	if macroIndex == 0 then -- Macro doesn't exist, create a new one
		...
		-- Updating an unchanged macro broadcasts UPDATE_MACROS and forces every
		-- action button to refresh.  Icon return values are not consistently
		-- normalized between Classic clients, so body equality is the reliable
		-- no-op test here.
...
-- Applies macro work that was requested during combat.  Keeping this replay in
-- PLAYER_REGEN_ENABLED avoids triggering UPDATE_MACROS while action buttons
-- are protected.
	...
	MagicButton:SetScript("OnEnter", function(self)
		...
		if itemID then
			...
			if exceptionItemIDs[itemID] then
				...
				elseif itemID == 4382 then		--elseif itemID == 30817 then
					RQEShowWrappedMacroTooltip(self, "Purchase Item(s)", macroBody)
					--GameTooltip:SetText("Purchase Item(s)\n\n" .. macroBody, nil, nil, nil, nil, true)
				...
				elseif itemID == 2058 then		--elseif itemID == 23784 then
