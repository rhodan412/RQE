...
EventManager.lua
Classic event registration, quest-state routing, combat deferrals, and frame coordination

]]


--------------------------------------------------
-- #1. 🌐 Namespace & Persistent Event State
--------------------------------------------------

	-------------------------------------------------------
	-- #1a. Addon Namespace & Persistent Quest State
	-------------------------------------------------------
	...
	-- A missing searched quest is nil, never a table. Repair the old empty-table
	-- placeholder before passing the saved value to quest APIs.
	RQECharacterDB = RQECharacterDB or {}
	RQECharacterDB.searchedQuestID = tonumber(RQECharacterDB.searchedQuestID)
	RQECharacterDB.uiInfoMessageCompletedObjectives = RQECharacterDB.uiInfoMessageCompletedObjectives or {}
	RQE.UIInfoMessageCompletedObjectives = RQECharacterDB.uiInfoMessageCompletedObjectives

--------------------------------------------------
-- #2. ⏱️ Event Frames & Timing State
--------------------------------------------------

	-------------------------------------------------------
	-- #2a. Dispatcher Frames & Map Update Timer
	-------------------------------------------------------
...
--------------------------------------------------
-- #3. 📡 Event Registration Catalog
--------------------------------------------------

	-------------------------------------------------------
	-- #3a. Registered Event Catalog
	-------------------------------------------------------
...
--------------------------------------------------
-- #4. 🧩 Event Step Predicates
--------------------------------------------------

	-------------------------------------------------------
	-- #4a. Current-Step Objective Status Detection
	-------------------------------------------------------
...
--------------------------------------------------
	...
	local function HandleEvents(frame, event, ...)
		...
		local handlers = {
			...
			PLAYER_LEVEL_UP = RQE.handlePlayerLevelUp,
			...
			SUPER_TRACKED_QUEST_CHANGED = RQE.handleSuperTracking,
	...
	-------------------------------------------------------
	-- #5c. Achievement, Content, Criteria & Mission Events
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5e. Inventory, Bag, Mail & Merchant Events
	-------------------------------------------------------

	-- Process item-count changes and re-evaluate inventory-driven quest steps when relevant.
	...
	function RQE.BagNewItemsAdded()
		...
		-- Return whether the current step contains a direct or grouped inventory requirement.
		local function stepUsesInventoryCheck(step)
	...
	-------------------------------------------------------
	-- #5f. Combat Lockdown & Deferred Replay
	-------------------------------------------------------
	...
	function RQE.handlePlayerRegenEnabled()
		...
		-- Complete a Classic quest-log selection that was deferred while in combat.
		local pendingQuestID = tonumber(RQE.PendingClassicQuestLogDetails)
		if pendingQuestID and RQE.DisplayClassicQuestLogDetails then
			RQE.PendingClassicQuestLogDetails = nil
			C_Timer.After(0.1, function()
				if not InCombatLockdown() then
					RQE:DisplayClassicQuestLogDetails(pendingQuestID)
				end
			end)
		end
		...
		-- Replay automatic work only after combat has fully ended.  These requests
		-- are set by the helpers that would otherwise synthesize clicks while
		-- Blizzard's protected action-bar state is locked.
		if RQE.ReClickQuestLogIndexButtonAfterCombat then
			local deferredQuestID = RQE.QuestButtonToReClickAfterCombat
			RQE.ReClickQuestLogIndexButtonAfterCombat = false
			RQE.QuestButtonToReClickAfterCombat = nil

			if deferredQuestID then
				C_Timer.After(0.1, function()
					if not InCombatLockdown() then
						RQE.ClickQuestLogIndexButton(deferredQuestID)
					end
				end)
			end
		end
		...
		if RQE.RunPeriodicChecksAfterCombat then
			RQE.RunPeriodicChecksAfterCombat = false
		...
		if RQE.ReapplyMacroBindingAfterCombat then
			RQE.ReapplyMacroBindingAfterCombat = false
		...
		if RQEMacro and RQEMacro.ProcessPendingMacroOperations then
			C_Timer.After(0.1, function()
				if not InCombatLockdown() then
					RQEMacro:ProcessPendingMacroOperations()
				end
			end)
		end

		if RQE.RefreshMagicButtonAfterCombat then
			RQE.RefreshMagicButtonAfterCombat = false
			...
			C_Timer.After(0.1, function()
				...
				if RQE.Buttons and RQE.Buttons.UpdateMagicButtonIcon then
		...
		if RQE.ToggleRQEFrameFromSlashAfterCombat then
			RQE.ToggleRQEFrameFromSlashAfterCombat = false
			...
			C_Timer.After(0.1, function()
				...
				RQE:SlashCommand("frame")
			end)
		end

		if RQE.ToggleRQEFrameAfterCombat then
			RQE.ToggleRQEFrameAfterCombat = false
		...
		if RQE.ToggleFramesAndTrackerAfterCombat then
			RQE.ToggleFramesAndTrackerAfterCombat = false

			C_Timer.After(0.1, function()
				if InCombatLockdown() then
					RQE.ToggleFramesAndTrackerAfterCombat = true
					return
				end
		...
		if RQE.WorldQuestButtonToClickAfterCombat then
			local deferredQuestID = RQE.WorldQuestButtonToClickAfterCombat
			RQE.WorldQuestButtonToClickAfterCombat = nil

			C_Timer.After(0.1, function()
				if InCombatLockdown() then
					RQE.WorldQuestButtonToClickAfterCombat = deferredQuestID
					return
				end
				RQE:ClickWorldQuestButton(deferredQuestID)
			end)
		end
	...
	-------------------------------------------------------
	-- #5g. Player Lifecycle, Map & Addon Events
	-------------------------------------------------------
	...
	function RQE.handlePlayerMountDisplayChanged()
		...
		if RQE.db.profile.autoClickWaypointButton then
			...
			local questID = RQE.searchedQuestID or extractedQuestID or RQE.API.GetSuperTrackedQuestID()	--C_SuperTrack.GetSuperTrackedQuestID()
			if RQE.db.profile.debugLevel == "INFO+" and RQE.db.profile.showPlayerMountDisplayChanged then
	...
	-- Handles the PLAYER_LEVEL_UP event function
	-- Fired when a player levels up. 
	function RQE.handlePlayerLevelUp(...)
		local event = select(2, ...)
		local level = select(3, ...)
		local healthDelta = select(4, ...)
		local powerDelta = select(5, ...)
		local numNewTalents = select(6, ...)
		local numNewPvpTalentSlots = select(7, ...)
		local strengthDelta = select(8, ...)
		local agilityDelta = select(9, ...)
		local staminaDelta = select(10, ...)
		local intellectDelta = select(11, ...)
	...
	function RQE.handlePlayerLogin()
		...
		-- Quest events can fire during login before the delayed restore below. Keep
		-- SavedVariables intact until RQE has restored its tracker mode and focus.
		RQE.PendingTrackerStateRestore = RQECharacterDB and (
			RQECharacterDB.trackedQuests ~= nil
			or RQECharacterDB.classicVirtualQuestWatches ~= nil
			or RQECharacterDB.classicManualVirtualQuestWatches ~= nil
			or RQECharacterDB.superTrackedQuestID ~= nil
			or RQECharacterDB.searchedQuestID ~= nil
		) or false
		...
		-- ProfileManager schedules character-state restoration after world entry,
		-- independently of errors in these gameplay initialization calls.
		...
		-- AceDB is initialized once by OnInitialize; never construct a placeholder DB here.
		...
		-- Use AceDB's actual selection, including a deliberately assigned Default.
		RQE:RequestProfileApply()
		if RQE.db.profile.debugLevel == "INFO+" then
			DEFAULT_CHAT_FRAME:AddMessage("Debug: Active profile: " .. RQE.db:GetCurrentProfile(), 0.68, 0.85, 0.9)
		end
	...
	-------------------------------------------------------
	-- #5h. Scenario, Group & Timer Events
	-------------------------------------------------------
	...
	function RQE.handleWorldStateTimerStart(...)
		...
		RQE.updateScenarioUI()
		-- print("~~~ UpdateRQEQuestFrame(): 1858 ~~~")
		UpdateRQEQuestFrame()	-- Updates RQEQuestFrame when Scenario Frame update fires (possible duplicate)
	end
	...
	function RQE.handleJailersUpdate(...)
		...
		C_Timer.After(4, function()
			RQE.updateScenarioUI()
			-- print("~~~ UpdateRQEQuestFrame(): 1924 ~~~")
			UpdateRQEQuestFrame()	-- Updates RQEQuestFrame when Scenario Frame update fires (possible duplicate)
			if RQE.db.profile.debugLevel == "INFO+" and RQE.db.profile.JailorsTowerLevelUpdate then
	...
	-------------------------------------------------------
	-- #5i. Navigation, World Geometry & Movement Events
	-------------------------------------------------------

	-- Reconcile minimap zoom transitions and refresh navigation state when indoor/outdoor scaling changes.
		...
		if isZoneChangeCheck then
			if questID then
				...
				C_Timer.After(1.3, function()
					...
					RQE:StartPeriodicChecks()	-- Checks 'funct' for current quest in DB after MINIMAP_UPDATE_ZOOM fires
				end)
			end
	...
	function RQE.handlePlayerControlGained()
		...
		if not OnTaxi then
			if not RQE.hoveringOnFrame then
				RQE.SortOnly = true
				-- print("~~~ UpdateRQEQuestFrame(): 1913 ~~~")
		...
		-- print("~~~ SortQuestsByProximity(): 1924 ~~~")
	...
	function RQE.handlePlayerStartedMoving()
		...
		-- Refresh the tracker once when movement starts, provided doing so is safe.
		-- Ongoing label updates and throttled re-sorts are handled by OnUpdate.
		if not InCombatLockdown() and not C_Scenario.IsInScenario() then
		...
		-- if not OnTaxi and not isResting then	-- Heavy Lag
			-- if not RQE.hoveringOnFrame then
				-- RQE.SortOnly = true
				-- -- print("~~~ UpdateRQEQuestFrame(): 1984 ~~~")
	...
	function RQE.handlePlayerStoppedMoving()
		...
		-- if not isInRaid and groupSize < 6 then
			-- SortQuestsByProximity()
		-- end
		-- The regular movement refresh is intentionally throttled. When movement
		-- stops, do one final immediate rebuild so the visible row order precisely
		-- matches the last displayed distances.
		if not InCombatLockdown() and not C_Scenario.IsInScenario() then
			C_Timer.After(0.05, function()
				if InCombatLockdown() then return end
				...
				-- RQE:RefreshTrackerOrderAfterMovement()
				-- Sort the backing list now, but do not clear/redraw the tracker yet.
				-- Auto Track below may change the active quest and waypoint; rendering
				-- before and after that change was the source of the visible stutter.
				if RQE.SortWatchedQuestsByProximity then
					RQE:SortWatchedQuestsByProximity()
				end

				-- Refresh the quest tracker after a movement-triggered proximity sort when combat permits it.
				local function renderStoppedMovementTracker()
					if not InCombatLockdown() then
						UpdateRQEQuestFrame()
					end
				end
		...
		-- if not isInRaid and groupSize < 6 then
			-- if not C_Scenario.IsInScenario() then
				-- if not InCombatLockdown() then
					-- C_Timer.After(0.25, function()
						-- RQE.isCheckingMacroContents = true
						-- RQEMacro:CreateMacroForCurrentStep()	-- Checks for macro status if PLAYER_STOPPED_MOVING event fires
						-- C_Timer.After(0.3, function()
							-- RQE.isCheckingMacroContents = false
						-- end)
					-- end)
	...
	function RQE.handleOverrideActionBar()
		-- Classic Era does not use the Retail override-action-bar flow.  More
		-- importantly, this event fires while Blizzard is updating protected action
		-- buttons, so RQE must not start UI or macro work from it on this client.
		...
		if isBuffOrDebuffCheck then
			...
			C_Timer.After(0.2, function()
				...
				RQE.ClickQuestLogIndexButton(RQE.API.GetSuperTrackedQuestID())	--C_SuperTrack.GetSuperTrackedQuestID())
				if questID then
	...
	-------------------------------------------------------
	-- #5j. Mouseover, Shapeshift, Widget & World Entry Events
	-------------------------------------------------------
	...
	function RQE.handleVariablesLoaded()
		...
		-- Restore both frames together once the world is ready and combat permits.
		RQE:RequestProfileApply()
		...
		-- Pending restoration retries after combat using the currently active profile.
		RQE:RequestProfileApply()
	end
	...
	function RQE.handlePlayerEnterWorld(...)
		...
		C_Timer.After(10, function()
			...
			if RQE.HasLoggedIn or RQE.HasReloaded then
				-- Refreshing the display is not a user-requested tracking reset.
				-- Protect the saved focus/watch state until the paired restore finishes.
		...
		if RQE.API.IsSuperTrackingQuest() then
		--if C_SuperTrack.IsSuperTrackingQuest() then
			local qid = RQE.API.GetSuperTrackedQuestID()	--C_SuperTrack.GetSuperTrackedQuestID()
		...
		if RQE and RQE.GetStepCoordinates and RQE.API.IsSuperTrackingQuest() then	--if RQE and RQE.GetStepCoordinates and C_SuperTrack.IsSuperTrackingQuest() then
			local sidx = RQE.AddonSetStepIndex or 1
				...
				if RQE.db.profile.autoClickWaypointButton then
					...
					if questID then
						C_Timer.After(0.35, function()
							...
							local zoneCheck = step and RQE:StepUsesZoneChange(step)

							-- determine if periodic checks should run
							local objChanged = RQE:DidObjectivesChange(questID)

							if objChanged or zoneCheck then
								if RQE.db.profile.debugLevel == "INFO+" then
									print("PLAYER_ENTERING_WORLD → StartPeriodicChecks() (" ..
										(objChanged and "objective-change" or "zone-change") .. ")")
								end
							...
							elseif RQE.db.profile.debugLevel == "INFO+" then
								print("PLAYER_ENTERING_WORLD → no objective/zone change → skipping StartPeriodicChecks()")
							end
						end)
					end
				end
	...
	-------------------------------------------------------
	-- #5k. Super-Tracking, Quest Acceptance, Vehicle & Zone Events
	-------------------------------------------------------

	-- Synchronize RQE quest, frame, waypoint, and macro state with Blizzard super-tracking changes.
	function RQE.handleSuperTracking()
		...
		--[[
		-- Previous Classic-specific direct-render recovery attempt.  It introduced
		-- a competing event path rather than preventing the later clear operation
		-- from blanking the main helper.
		local isClassicEra = tonumber((GetBuildInfo()):match("^(%d+)")) == 1
		if isClassicEra and newQID and RQE.DisplayClassicQuestLogDetails then
			local classicQuestID = newQID
			C_Timer.After(0, function()
				if tonumber(RQE.API.GetSuperTrackedQuestID()) == tonumber(classicQuestID) then
					RQE:DisplayClassicQuestLogDetails(classicQuestID)
				end
			end)
		end
		]]
		...
		if tonumber(newQID) and tonumber(newQID) > 0
			...
			-- Redraw after a same-quest row reselect without resetting its DB step.
		...
		if RQE.RQEQuestFrame and not RQE.RQEQuestFrame:IsShown() then
			...
			--if not C_SuperTrack.IsSuperTrackingQuest() then
				...
				superQuestID = RQE.API.GetSuperTrackedQuestID()	--C_SuperTrack.GetSuperTrackedQuestID()
			end
			...
			-- The row click's transient clear must not lose its selected step.
		...
		if RQE.currentSuperTrackedQuestID ~= RQE.previousSuperTrackedQuestID then
			...
			-- A matching, explicitly reselected quest keeps its actual DB step.
			if not reselectPending then
				RQE.LastClickedIdentifier = nil
				RQE.CurrentStepIndex = 1
				RQE.AddonSetStepIndex = 1
			...
			if questID then
				C_Timer.After(0.25, function()
					...
					if newQID ~= oldQID then
						...
						-- Otherwise, ignore SUPER_TRACKING_CHANGED for objective progress
						if RQE.db.profile.debugLevel == "INFO+" then
							print("SUPER_TRACKING_CHANGED → Ignoring (quest did not change)")
						end
					end
				end)
			end
		...
		if RQE.ManualSuperTrack then
			if RQE.ManualSuperTrack ~= "BQ" then
				--[[
				-- Previous Classic-specific suppression.  The normal clear remains
				-- correct now that the later active-supertrack clear guard preserves
				-- the selected Classic quest.
				if not isClassicEra then
					RQE:ClearFrameData()
				end
				]]
				--[[
				-- Previous unconditional clear.  When a Classic tracker button has
				-- already rendered this same quest, this erased only the main helper
				-- before the generic updater could decide that nothing changed.
				RQE:ClearFrameData()
				]]
				local currentQuestID = tonumber(newQID)
				local displayedQuestID = tonumber(RQE.DisplayedQuestID)
				-- local isCurrentClassicRender =
				-- 	tonumber((GetBuildInfo()):match("^(%d+)")) == 1
				local isCurrentClassicRender =
					RQE.API.GameVersion and RQE.API.GameVersion.usesLegacyQuestLog
					and currentQuestID
					and displayedQuestID == currentQuestID
					and RQE.API.IsOnQuest(currentQuestID)

				-- Preserve only a main helper that already belongs to the active
				-- Classic selection.  A real quest switch (or a stale/blank helper)
				-- still takes the existing clear path.
				if not isCurrentClassicRender then
					RQE:ClearFrameData()
				end
			end
	...
	function RQE.handleQuestAccepted(...)
		local event = select(2, ...)
		-- Parentheses prevent select() from expanding the remaining event payload
		-- into tonumber's optional second "base" argument.
		local acceptedQuestLogIndex = tonumber((select(3, ...)))
		local questID = tonumber((select(4, ...)))

		-- QUEST_ACCEPTED supplies questLogIndex first and questID second. Resolve
		-- through the log as a fallback for Classic client variations that omit ID.
		if not questID and acceptedQuestLogIndex then
		...
		-- the accepted quest to the quest log and updating its zone information.
		if RQE.db.profile.autoTrackZoneQuests then
			C_Timer.After(2.5, function()
				RQE.DisplayCurrentZoneQuests()
			end)
		...
		if questID == RQE.searchedQuestID then
			-- A searched quest's virtual pickup step (0) ends as soon as the quest
			-- enters the player's log. Resume normal objective-driven progression at
			-- step 1; subsequent checks may advance it further if already complete.
			if RQE.ManualPreviewQuestID == questID then
		...
		if questID then
			...
			local watchType = RQE.API.GetQuestWatchType(questID)
			local isManuallyTracked = Enum and Enum.QuestWatchType and watchType == Enum.QuestWatchType.Manual  -- Applies when world quest is manually watched and then accepted when player travels to world quest spot
			...
			C_Timer.After(0.25, function()
				if not RQE.API.GetLogIndexForQuestID(questID) then return end

				if RQE.API.GameVersion and RQE.API.GameVersion.usesLegacyQuestLog then
					RQE:RememberAutoQuestWatchProgress(questID)
					RQE:AutoWatchQuest(questID)
				elseif isWorldQuest and not isManuallyTracked then
			...
			C_Timer.After(0.5, function()
				...
				if RQE.ManualSuperTrack then
					...
					if superTrackIDToApply and superTrackIDToApply ~= RQE.API.GetSuperTrackedQuestID() then	--C_SuperTrack.GetSuperTrackedQuestID() then
						...
						RQE.API.SetSuperTrackedQuestID(superTrackIDToApply)
	...
	function RQE.handleZoneChange(...)
		...
		if event == "ZONE_CHANGED" or event == "ZONE_CHANGED_INDOORS" then
			...
			--if C_SuperTrack.IsSuperTrackingQuest() then
				...
				if RQE:DidDirectionTextChange(qid) then
					if RQE.db.profile.debugLevel == "INFO+" then
					...
					if RQE.db.profile.debugLevel == "INFO+" then
						print("ZONE_CHANGED → no direction change → skipping UpdateFrame()")
					end
					RQE.DirectionChangedUpdateRQEFrame = false
				end
			...
			-- Optional: auto-refresh stash so tooltips/arrow update without /reload
			if RQE and RQE.GetStepCoordinates and RQE.API.IsSuperTrackingQuest() then
	...
	function RQE.handleZoneNewAreaChange()
		...
		-- Refresh RQE's zone-only list independently of supertracking. Keeping this
		-- near the start ensures it still runs when the handler returns early later.
		if RQE.db.profile.autoTrackZoneQuests then
			C_Timer.After(0.5, function()
				RQE.DisplayCurrentZoneQuests()
			end)
		...
		if not OnTaxi and not isResting then
			if not RQE.hoveringOnFrame then
				RQE.SortOnly = true
				-- print("~~~ UpdateRQEQuestFrame(): 3425 ~~~")
		...
		-- if not UnitOnTaxi("player") and not RQE.isDragonRiding then
			C_Timer.After(1.5, function()
				...
				-- if RQE.db.profile.autoTrackZoneQuests then
					-- RQE.DisplayCurrentZoneQuests()
				-- end
				-- Zone-quest refresh now runs at the start of this handler, before
				-- any supertrack-dependent early return can bypass it.

				-- print("~~~ SortQuestsByProximity(): 3516 ~~~")
				SortQuestsByProximity()
		...
		-- The delayed DisplayCurrentZoneQuests() above is the single ZQ rebuild for
		-- this map transition. Calling filterByZone() here performed a second redraw
		-- and could clear/reapply zone state before Classic finished updating maps.
		--[[
	...
	-------------------------------------------------------
	-- #5l. Recipe, UI Message, Aura, Model & Spell Events
	-------------------------------------------------------

	-- Process tracked-recipe changes that can affect profession-related quest progress.
	...
	-- Normalizes UI_INFO_MESSAGE objective text so the " (Complete)" suffix sent by
	-- the client matches the text returned by the quest-objective API.
	function RQE:NormalizeUIInfoMessageObjectiveText(text)
		if type(text) ~= "string" then
			return nil
		end

		text = text:gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", "")
		text = text:gsub("%s+%(%s*Complete%s*%)%s*$", "")
		return text:gsub("^%s+", ""):gsub("%s+$", "")
	end

	-- Check whether a normalized UI information message marked this quest objective complete.
	function RQE:IsUIInfoMessageObjectiveComplete(questID, objectiveText)
		local completedObjectives = self.UIInfoMessageCompletedObjectives
		local normalizedText = self:NormalizeUIInfoMessageObjectiveText(objectiveText)
		return completedObjectives
			and completedObjectives[tonumber(questID)]
			and normalizedText
			and completedObjectives[tonumber(questID)][normalizedText]
	end
	...
	function RQE.handleUIInfoMessage(...)
		...
		-- Some Classic objectives report completion through UI_INFO_MESSAGE 295 without
		-- updating the objective API's finished flag. Remember the matching objective
		-- text, rather than marking every objective on the quest complete.
		if tonumber(messageType) == 295 then
			local questID = tonumber(RQE.DisplayedQuestID) or tonumber(RQE.API.GetSuperTrackedQuestID())
			local objectiveText = RQE:NormalizeUIInfoMessageObjectiveText(message)
			if questID and objectiveText then
				-- Store this per character so a reload does not lose the completion
				-- indication when Classic continues to report finished = false.
				RQECharacterDB.uiInfoMessageCompletedObjectives = RQECharacterDB.uiInfoMessageCompletedObjectives or {}
				RQE.UIInfoMessageCompletedObjectives = RQECharacterDB.uiInfoMessageCompletedObjectives
				RQE.UIInfoMessageCompletedObjectives[questID] = RQE.UIInfoMessageCompletedObjectives[questID] or {}
				RQE.UIInfoMessageCompletedObjectives[questID][objectiveText] = true
		...
		if messageType ~= 308 and messageType ~= 309 and messageType ~= 310 and messageType ~= 311 and messageType ~= 312 and messageType ~= 313 and messageType ~= 314 then
			...
			if RQE.db.profile.debugLevel == "INFO+" then
				print("(Valid) messageType is: " .. messageType)
				print("(Valid) message is: " .. tostring(message))
			end
		end
	...
	function RQE.handleUnitAura(...)
		...
		if hasChangedOrRemovedAura then
			-- Detect combined amount expressions that add the live quest-objective count.
			...
			-- Determine whether any quest step combines a buff count with objective progress.
	...
	-------------------------------------------------------
	-- #5m. Unit Quest Log, Scenario UI & Quest Status Events
	-------------------------------------------------------

	-- Re-evaluate quest data and displayed steps when the player's quest log changes.
	function RQE.handleUnitQuestLogChange(...)
		...
		-----------------------------------------
		-- CONDITIONAL BLOCK: Only if player + quest
		-----------------------------------------
		local questID = RQE.API.GetSuperTrackedQuestID()	--local questID = C_SuperTrack.GetSuperTrackedQuestID()
				...
				C_Timer.After(0.2, function()
					...
					if RQE.WaypointButtons and RQE.AddonSetStepIndex then
						if RQE.PeriodicIsFiring then
							...
							if btn then
								if RQE.db.profile.debugLevel == "INFO+" then
									print("Clicking RQE.WaypointButtons[RQE.AddonSetStepIndex] due to changes detected")
								end
								btn:Click()
							else
								if RQE.db.profile.debugLevel == "INFO+" then
									print("UQLC: Missing button for AddonSetStepIndex")
								end
							end
							RQE.PeriodicIsFiring = false
						end
	...
	function RQE.updateScenarioCriteriaUI()
		...
		-- print("~~~ UpdateRQEQuestFrame(): 4141 ~~~")
	...
	function RQE.updateScenarioUI()
		...
		if not mythicMode then
			...
			-- print("~~~ UpdateRQEQuestFrame(): 4221 ~~~")
			UpdateRQEQuestFrame()	-- Updates RQEQuestFrame when Scenario Frame update fires (possible duplicate)
	...
	function RQE.handleInstanceInfoUpdate()
		...
		if RQE.db.profile.autoClickWaypointButton then
			...
			-- Only run if we HAVE a supertracked quest
			if questID then
				...
				elseif RQE.db.profile.debugLevel == "INFO+" then
					print("UPDATE_INSTANCE_INFO: No objective change → skipping StartPeriodicChecks()")
				end
			end
		end
	...
	function RQE.handleQuestStatusUpdate()
		if RQE.API.GameVersion and RQE.API.GameVersion.usesLegacyQuestLog then
			-- RQE:AutoWatchQuestsWithChangedObjectives()
			-- Objective values may still be stale during this event on Classic.
			-- Queue the comparison after the quest-log refresh so partial progress
			-- (for example 4/10 -> 5/10) is never missed.
			RQE:QueueAutoWatchObjectiveCheck()

			-- Classic can set the quest-complete flag after QUEST_LOG_UPDATE has
			-- begun. Recheck on the next frame and only run step progression when the
			-- database explicitly opts into the final CheckDBComplete turn-in step.
			if not RQE.ClassicTurnInStepCheckQueued then
				RQE.ClassicTurnInStepCheckQueued = true
				C_Timer.After(0.15, function()
					RQE.ClassicTurnInStepCheckQueued = false
					local questID = RQE.API.GetSuperTrackedQuestID()
					local questData = questID and RQE.getQuestData(questID)
					local hasCheckDBComplete = questData and RQE:HasCheckDBComplete(questData)
					if hasCheckDBComplete and RQE:IsQuestReadyForTurnIn(questID) then
						RQE:QueuePeriodicChecks("CLASSIC_QUEST_READY_FOR_TURN_IN", 0, questID)
					end
				end)
			end
		end
			...
			C_Timer.After(0.7, function()
				if questID then
					if RQE.ManualSuperTrack and questID ~= RQE.ManualSuperTrackedQuestID then
						...
						RQE.API.SetSuperTrackedQuestID(RQE.ManualSuperTrackedQuestID)
		...
		-- Classic can fire QUEST_LOG_UPDATE before the quest tracker has refreshed its
		-- objective text. Refresh once shortly afterward so watched quests show the
		-- current progress without rebuilding the tracker for every event in a burst.
		if not RQE.TrackerRefreshQueued then
			RQE.TrackerRefreshQueued = true
			C_Timer.After(0.15, function()
				RQE.TrackerRefreshQueued = false
				if UpdateRQEQuestFrame then
					UpdateRQEQuestFrame()
				end
			end)
	...
	-------------------------------------------------------
	-- #5n. Quest Reward, Criteria, Loot & Quest-Line Events
	-------------------------------------------------------

	-- Recheck quest progress when a quest-related currency reward is received.
	...
	function RQE.handleQuestLogCriteriaUpdate(...)
		...
		if isSuperTracking or RQE.isSuperTracking then
			local superQuestID = RQE.API.GetSuperTrackedQuestID()	--C_SuperTrack.GetSuperTrackedQuestID()
	...
	-------------------------------------------------------
	-- #5o. Quest Completion, Scene & Removal Events
	-------------------------------------------------------

	-- Prepare RQE state when the player reaches a quest's completion dialog.
	...
	function RQE.handleQuestRemoved(...)
		...
		-- 	if RQE.HiddenQuestLogQuests then
		-- 		RQE.HiddenQuestLogQuests[questID] = nil
		-- 	end
		-- end
		if questID then
			RQE:RemoveClassicVirtualQuestWatch(questID)
			if RQE.UIInfoMessageCompletedObjectives then
				RQE.UIInfoMessageCompletedObjectives[questID] = nil
			end
			if RQE.HiddenQuestLogQuests then
				RQE.HiddenQuestLogQuests[questID] = nil
			end
			RQE:RemoveInactiveQuestTrackerSearchResult(questID)
		end
	...
	-------------------------------------------------------
	-- #5p. Quest Watch Synchronization Events
	-------------------------------------------------------

	-- Synchronize watched-quest state and objective progress after a quest-watch update.
	function RQE.handleQuestWatchUpdate(...)
		...
		-- QUEST_WATCH_UPDATE is Classic's direct notification that this quest's
		-- objective changed. Queue an RQE-only virtual watch after objective data
		-- has settled; no Blizzard quest watch is created.
		if RQE.API.GameVersion and RQE.API.GameVersion.usesLegacyQuestLog then
			RQE:QueueVirtualAutoWatchFromProgressEvent(questID)
		end
		...
		if isSuperTracking and RQE.currentSuperTrackedQuestID == questID then
			...
			if RQE.currentSuperTrackedQuestID then
				superTrackedQuestName = RQE.API.GetTitleForQuestID(RQE.currentSuperTrackedQuestID) or "Unknown Quest"
			end
		...
		elseif not isSuperTracking then
			...
			if RQE.db.profile.debugLevel == "INFO+" and RQE.db.profile.QuestWatchUpdate then
				local superTrackedQuestName = RQE.API.GetTitleForQuestID(RQE.currentSuperTrackedQuestID) or "Unknown Quest"
		...
		-- RQE:QuestType()
		-- -- print("~~~ SortQuestsByProximity(): 5032 ~~~")
		-- SortQuestsByProximity()
		...
		-- 	C_QuestLog.AddQuestWatch(questID)
		-- end
		-- QUEST_WATCH_UPDATE now queues RQE:QueueVirtualAutoWatchFromProgressEvent()
		-- instead, keeping automatic progress tracking outside Blizzard's tracker.
		...
		-- Determine questID, questInfo, StepsText, CoordsText and MapIDs based on various fallbacks
		local advanceQuestID =	RQE.searchedQuestID	or extractedQuestID	or (isSuperTracking and RQE.currentSuperTrackedQuestID)	or questID or RQE.API.GetSuperTrackedQuestID()	--C_SuperTrack.GetSuperTrackedQuestID()
			...
			if RQE.db.profile.autoClickWaypointButton then
				C_Timer.After(0.25, function()
					...
					if RQE:DidObjectivesChange(advanceQuestID) then
						...
						if RQE.db.profile.debugLevel == "INFO+" then
							print("QWU: No objective change → skipping StartPeriodicChecks()")
						end
					end
	...
	function RQE.handleQuestWatchListChanged(...)
		...
		if not questID then
			-- Classic can fire QUEST_WATCH_LIST_CHANGED without a quest payload.
			-- Persist the current RQE/Blizzard tracker state even in that form.
			C_Timer.After(0.15, function()
				RQE:SaveTrackedQuestsToCharacter()
		...
		-- Mirror manual Blizzard watch-list changes in RQE's own virtual list.
		-- This does not add or remove a Blizzard quest watch; it only makes the
		-- quest appear or disappear in RQEQuestFrame with the event's true/false.
		if RQE.API.GameVersion and RQE.API.GameVersion.usesLegacyQuestLog then
			if added == true then
				-- RQE:AddClassicVirtualQuestWatch(questID)
				-- Preserve an explicit Quest Log watch as its own RQE virtual source.
				-- ZQ combines this with its current-zone source without treating other
				-- auto/progress virtual watches as cross-zone manual watches.
				RQE:AddClassicManualVirtualQuestWatch(questID)
			elseif added == false then
				RQE:RemoveClassicVirtualQuestWatch(questID)
				-- SC/ZQ intentionally enumerate quests beyond Blizzard's watch list.
				-- Preserve the player's explicit unwatch there as an RQE-only hide.
			...
			-- A false event may arrive after the virtual key was already absent.
			-- Always rebuild after Blizzard's native watch list settles.
			RQE:QueueClassicVirtualQuestTrackerRefresh()
		end
		...
		if RQE.UnitQuestLogChangedFired then	-- may need to place this flag in other locations too to ensure that RQEQuestFrame updates, but keep lag low
			...
			if isWorldQuest then
				...
				-- print("~~~ UpdateRQEQuestFrame(): 5183 ~~~")
			...
			if RQE.QuestAddedForWatchListChanged and not isSuperTracking then
				...
				if not isWorldQuest then
					...
					RQE.API.SetSuperTrackedQuestID(questID)	-- If still nothing is being supertracked the addon will opt to super track the quest that fired the event
					RQE:SaveSuperTrackedQuestToCharacter()	-- Saves the character's currently supertracked quest when QUEST_WATCH_LIST_CHANGED event fires
					UpdateFrame()
				end
			end
	...
	-------------------------------------------------------
	-- #5q. Turn-In, Detail, Finish & Logout Events
	-------------------------------------------------------

	-- Clean up tracking, waypoints, and cached state after a quest is turned in.
	function RQE.handleQuestTurnIn(...)
		...
		if questID then
			...
			-- QUEST_REMOVED normally follows shortly after turn-in. Retry after the
			-- log has settled in case that event is delayed or absent.
			C_Timer.After(0.15, function()
				RQE:RemoveInactiveQuestTrackerSearchResult(questID)
			end)
		end
		...
		if RQE.db.profile.debugLevel == "INFO+" and RQE.db.profile.QuestTurnedIn then
			DEFAULT_CHAT_FRAME:AddMessage("QTI 02 Debug: SuperTrackedQuestID: " .. tostring(RQE.API.GetSuperTrackedQuestID()), 1.0, 0.08, 0.58)
	...
	function RQE.handleQuestDetail(...)
		...
		-- QUEST_DETAIL fires before an offered quest is added to the quest log, so
		-- GetTitleForQuestID cannot yet resolve its title. GetTitleText is the
		-- Classic API for the quest currently displayed by the quest giver.
		local questName = (type(GetTitleText) == "function" and GetTitleText())
			or RQE.API.GetTitleForQuestID(questID)
			or "Unknown Quest"

		-- Prefer the level from the quest log when the quest is already present
		-- (for example, a repeatable quest). For a newly offered Classic quest,
		-- use the level embedded in Blizzard's displayed quest header when present.
		local questLevel
		local questLogIndex = RQE.API.GetLogIndexForQuestID(questID)
		if questLogIndex then
			local questInfo = RQE.API.GetQuestLogInfo(questLogIndex)
			questLevel = questInfo and tonumber(questInfo.level)
		end

		if not questLevel and type(GetQuestLevel) == "function" then
			questLevel = tonumber(GetQuestLevel())
		end

		-- Blizzard does not expose an offered quest's level through QUEST_DETAIL.
		-- When Questie is loaded, use the same Questie quest database that supplies
		-- the colored level in its NPC tooltip. This is optional: RQE still uses its
		-- native quest-log and displayed-header fallbacks when Questie is absent.
		local questieDB
		if C_AddOns and C_AddOns.IsAddOnLoaded and C_AddOns.IsAddOnLoaded("Questie")
			and QuestieLoader and type(QuestieLoader.ImportModule) == "function"
		then
			local success, module = pcall(QuestieLoader.ImportModule, QuestieLoader, "QuestieDB")
			if success and module and type(module.GetQuest) == "function" then
				questieDB = module
				if not questLevel then
					local questieQuest = module.GetQuest(questID)
					questLevel = questieQuest and tonumber(questieQuest.level)
				end
			end
		end

		if not questLevel and QuestInfoTitleHeader and QuestInfoTitleHeader.GetText then
			local displayedQuestTitle = QuestInfoTitleHeader:GetText()
			questLevel = displayedQuestTitle and tonumber(displayedQuestTitle:match("^%[(%d+)"))
		end

		-- GetQuestTagInfo can be queried for the quest currently displayed by the
		-- quest giver. Use its explicit tag name instead of the World Quest isElite
		-- flag, which is not reliable for normal Classic quests.
		local questTagName
		if type(GetQuestTagInfo) == "function" then
			local _, tagName = GetQuestTagInfo(questID)
			questTagName = tagName
		end
		if not questTagName and questieDB and type(questieDB.GetQuestTagInfo) == "function" then
			local _, tagName = questieDB.GetQuestTagInfo(questID)
			questTagName = tagName
		end
		...
		if questLevel and questLevel > 0 then
			...
			local levelSuffix = ""
			if questTagName == "Dungeon" then
				levelSuffix = "D"
			elseif questTagName == "Elite" then
				levelSuffix = "E"
			elseif questTagName == "PvP" or questTagName == "PVP" then
				levelSuffix = " PvP"
			end

			levelText = levelColor .. string.format("[%d%s]", questLevel, levelSuffix) .. "|r "
		end

		local messagePrefix = "QuestID (displayed): " .. tostring(questID) .. " - " .. levelText .. questName
		...
		if RQE.db.profile.debugLevel == "NONE" then
			...
			-- end
			DEFAULT_CHAT_FRAME:AddMessage(messagePrefix, 0.46, 0.82, 0.95) -- Sky Blue
			return
		end
	...
	function RQE.handlePlayerLogout()
		-- Save after the UI has settled so SC/ZQ's RQE-only state survives /reload.
		-- Leave a pending restore guarded on early logout; preserve its saved snapshot.
		RQE:SaveTrackedQuestsToCharacter()
		RQE:SaveSuperTrackedQuestToCharacter()
	...
	-------------------------------------------------------
	-- #5r. Dispatcher Binding & Event Registration Loop
	-------------------------------------------------------
...
--------------------------------------------------
-- #6. 🖱️ Search Callbacks & Blizzard Tracker Hooks
--------------------------------------------------

	-------------------------------------------------------
	-- #6a. Search Button & EditBox Callbacks
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #6b. Classic-Safe Blizzard Tracker Hooking
	-------------------------------------------------------

	-- Never create a placeholder named ObjectiveTrackerFrame. Blizzard owns that
	-- global and expects its real managed-frame implementation to be present.
	local function IsClassicEraClient()
		return RQE.API and RQE.API.IsLegacyClient and RQE.API.IsLegacyClient()
	end

	-- Install the supported Objective Tracker visibility hooks while avoiding protected Classic Era UI.
	local function HookBlizzardObjectiveTracker()
		-- Classic Era's ObjectiveTrackerFrame is managed and protected by Blizzard.
		-- Hooking or adding fields to it can taint unrelated protected UI, including
		-- the Game Menu and action bars.
		if IsClassicEraClient() then
			return
		end

		if not ObjectiveTrackerFrame or ObjectiveTrackerFrame.RQEVisibilityHooked then
			return
		end

		ObjectiveTrackerFrame.RQEVisibilityHooked = true
		ObjectiveTrackerFrame:HookScript("OnShow", function()
	...
	HookBlizzardObjectiveTracker()
	if C_Timer and C_Timer.After then
		-- Some Blizzard frames initialize after addon files are loaded.
		C_Timer.After(1, HookBlizzardObjectiveTracker)
	end


--------------------------------------------------
-- #7. 🖥️ Objective Tracker Visibility & Frame Coordination
--------------------------------------------------

	-------------------------------------------------------
	-- #7a. Objective Tracker Show, Hide & Toggle Controls
	-------------------------------------------------------

	-- Function to Show the Objective Tracker
	function ShowObjectiveTracker()
		if IsClassicEraClient() then return end
	...
	function RQE:ToggleFramesAndTracker()
		...
		if RQEFrame:IsShown() then
			...
			if not IsClassicEraClient() and ObjectiveTrackerFrame then
	...
	-------------------------------------------------------
	-- #7b. Map Hover Visibility Watchdog
	-------------------------------------------------------
