...
function AceConfigDialog:Open(appName, container, ...)
	-- Do not replace Blizzard's protected CloseSpecialWindows global.  RQE uses
	-- this library for embedded Blizzard options, which do not need the legacy
	-- standalone-window hook.  Replacing it taints the Escape/Game Menu path on
	-- Classic Era and can block Logout or Exit.
	local app = reg:GetOptionsTable(appName)
	if not app then
		error(("%s isn't registed with AceConfigRegistry, unable to open config"):format(appName), 2)
	end
