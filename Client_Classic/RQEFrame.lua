...
RQEFrame.lua
Classic Quest Helper frame construction, rendering, interaction, persistence, and focused-step display

]]


--------------------------------------------------
-- #1. 🌐 Namespace, Shared State & Compatibility
--------------------------------------------------

	-------------------------------------------------------
	-- #1a. Addon Namespace & Shared Frame Tables
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #1b. Lua Compatibility & Initial Frame State
	-------------------------------------------------------

	-- Lua 5.1 uses the global unpack function.  Keep the fallback local so RQE
	-- never writes to the shared table library.
	...
	-------------------------------------------------------
	-- #1c. Legacy Dropdown Menu Definition
	-------------------------------------------------------
...
--------------------------------------------------
-- #2. 👥 Group Tracking State
--------------------------------------------------

	-------------------------------------------------------
	-- #2a. Last-Known Group Size & Type
	-------------------------------------------------------
...
--------------------------------------------------
	...
	function ShowQuestDropdownRQEFrame(self, questID)
		...
		MenuUtil.CreateContextMenu(UIParent, function(ownerRegion, rootDescription)
			...
			rootDescription:CreateButton("Untrack Quest", function() RQE.API.RemoveQuestWatch(questID); RQE:ClearFrameData(); end)
			...
			local uiMapID = RQE.API.GetBestMapForUnit("player")
...
--------------------------------------------------
-- #5. 🎨 Main Quest Helper Construction & Layout
--------------------------------------------------

	-------------------------------------------------------
	-- #5a. Main Frame, Header & Scroll Container
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5b. Custom Scrollbar & Frame Click Handling
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5c. Quest Action Buttons & Button Tooltips
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #5d. Quest Identity Text & Objective Colorization
	-------------------------------------------------------
	...
	local function colorizeObjectives(questID)
		...
		for _, objective in ipairs(objectivesData) do
			...
			if objective.finished == true or objective.finished == 1 then
	...
	if RQE.db and RQE.db.profile and RQE.db.profile.textSettings then
		...
		if RQE.db.profile.textSettings.QuestIDText.color then
			RQE.QuestIDText:SetTextColor(TableUnpack(RQE.db.profile.textSettings.QuestIDText.color))
		end
	end
	...
	if QuestNameText_settings then
		...
		if QuestNameText_settings.color then
			RQE.QuestNameText:SetTextColor(TableUnpack(QuestNameText_settings.color))
		end
	end
	...
	-------------------------------------------------------
	-- #5e. Direction, Status & Legacy Quest Timers
	-------------------------------------------------------
	...
	if DirectionTextFrame_settings then
		...
		if DirectionTextFrame_settings.color then
			RQE.DirectionTextFrame:SetTextColor(TableUnpack(DirectionTextFrame_settings.color or {1, 1, 0.85}))
		end
	end
	...
	local function AnchorDirectionBelowQuestStatus(hasStatus)
		...
		elseif searchGroupShown then
			if themed then
				RQE.DirectionTextFrame:SetPoint("TOPLEFT", RQE.SearchGroupButton, "BOTTOMLEFT", 8, -12)
			else
				RQE.DirectionTextFrame:SetPoint("TOPLEFT", RQE.SearchGroupButton, "BOTTOMLEFT", 0, -20)
			end
		else
	...
	-- Cache and periodically resynchronize a legacy quest timer while deriving smooth seconds remaining locally.
	...
	-- Refresh the legacy-client quest failure or countdown status and reposition the direction row beneath it.
	...
	-------------------------------------------------------
	-- #5f. Description, Objectives & Adaptive Text Layout
	-------------------------------------------------------
	...
	if QuestDescription_settings then
		...
		if QuestDescription_settings.color then
			RQE.QuestDescription:SetTextColor(TableUnpack(QuestDescription_settings.color or {0, 1, 0.6}))
		end
	end
	...
	if QuestObjectives_settings then
		...
		if QuestObjectives_settings.color then
			RQE.QuestObjectives:SetTextColor(TableUnpack(QuestObjectives_settings.color or {0, 1, 0.6}))
		end
	end
	...
	-------------------------------------------------------
	-- #5g. Location Information & Performance Metrics
	-------------------------------------------------------
	...
	-- map API behavior.  It avoids relying solely on movement events.
	...
	-------------------------------------------------------
	-- #5h. Header Controls & Search Execution
	-------------------------------------------------------
...
--------------------------------------------------
-- #6. ⚡ Frame Events, Tooltips & Interaction
--------------------------------------------------

	-------------------------------------------------------
	-- #6a. Resize & Quest Navigation Events
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #6c. Quest Tooltip Construction
	-------------------------------------------------------

	-- Function to create tooltip for QuestID and QuestName
	local function CreateQuestTooltip(frame, questID)
		local effectiveQuestID = tonumber(RQE.searchedQuestID) or tonumber(questID)
		...
		extractedQuestID = tonumber(RQE.DisplayedQuestID)
		-- extractedQuestID = tonumber(RQE.QuestIDText:GetText():match("%d+"))

		questID = effectiveQuestID or extractedQuestID or tonumber(currentSuperTrackedQuestID)
		if not questID then
			GameTooltip:SetText("Unknown Quest")
			GameTooltip:Show()
			return
		end
		...
		if type(questTitle) ~= "string" or questTitle == "" then
			questTitle = questData and type(questData.title) == "string" and questData.title or "Unknown Quest"
		end
		...
		if RQE.API.IsOnQuest(questID) then
			...
			if IsInGroup() then
				...
				-- GetQuestPartyProgress is a Retail tooltip API and is not present in
				-- Classic Era.  Leave party-progress details out when it is unavailable.
	...
	-------------------------------------------------------
	-- #6e. Search, Width & Scroll Controls
	-------------------------------------------------------
...
--------------------------------------------------
-- #7. 🧩 Search, Step Rendering & Quest Actions
--------------------------------------------------

	-------------------------------------------------------
	-- #7a. Search Frame & Result Loading
	-------------------------------------------------------
	...
	function RQE.SearchModule:FetchAndDisplayQuestData(questID)
		...
		C_Timer.After(1, function()  -- Wait for 1 second for data to load
			...
			local questLogIndex = RQE.API.GetLogIndexForQuestID(questID) or questID
			local questDetail, questObjectives = RQE.API.GetQuestLogText(questLogIndex)
	...
	-------------------------------------------------------
	-- #7b. Step Text, Coordinate & Waypoint Rendering
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #7c. Step Progression & Objective Index Tracking
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #7d. Objective Completion & Unknown Quest Waypoints
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #7e. Group Finder & Roster Coordination
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #7f. Step Cleanup, Dropdown & Content Sizing
	-------------------------------------------------------
...
--------------------------------------------------
-- #8. 💾 Geometry Persistence & Separate Focus Systems
--------------------------------------------------

	-------------------------------------------------------
	-- #8a. Search Startup & Geometry Persistence
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #8c. Separate Focus Frame Layout & Content Rendering
	-------------------------------------------------------
	...
	function RQE.InitializeSeparateFocusFrame()
		...
		function RQE:UpdateSeparateFocusFrame()
			...
			local isSearchedStepPreview = searchedID
				and not RQE.API.IsOnQuest(searchedID)
			...
			-- A searched quest preview follows its selected DB step. All other
			-- contexts continue to follow the active Blizzard/RQE supertrack.
			if isSearchedStepPreview then
	...
	-------------------------------------------------------
	-- #8d. Separate Focus Waypoint Control
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #8e. Focus Tooltips & Coordinate Data
	-------------------------------------------------------
	...
	-------------------------------------------------------
	-- #8f. Frame Visibility Enforcement
	-------------------------------------------------------
