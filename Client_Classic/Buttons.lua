...
RQE.UnknownButtonTooltip = function()
	RQE.UnknownQuestButton:SetScript("OnEnter", function(self)
		...
		if searchedQuestID then
			...
			local isComplete = RQE.API.IsQuestFlaggedCompleted(searchedQuestID)
			local isInLog = RQE.API.GetLogIndexForQuestID(searchedQuestID)
			...
			if not isInLog then
				...
				elseif continentID then
					...
					local playerMapID = RQE.API.GetBestMapForUnit("player")
		...
		C_Timer.After(0.2, function()
			...
			if RQE.CurrentTrackedQuestID then  -- Add a check to ensure questID is not nil
				...
				local x, y = RQE.API.GetNextWaypointForMap(RQE.CurrentTrackedQuestID, mapID)
				RQE.WPxPos = x
				RQE.WPyPos = y
				if RQE.WPxPos ~= nil then
					...
					if RQE.db.profile.debugLevel == "INFO" then
						...
						local directionText = RQE.API.GetNextWaypointText(RQE.CurrentTrackedQuestID)
			...
			elseif not RQE.DatabaseSuperX and RQE.DatabaseSuperY or not RQE.superX or not RQE.superY and RQE.superMapID then
				-- Open the quest log details for the super tracked quest to fetch the coordinates	-- COMMENTED OUT AS IT WAS CAUSING COORDINATE TOOLTIP FROM BEING MISALIGNED ON INFO DEBUG WITH HANDYNOTES WORLDMAPBUTTON ADDON ACTIVE
				...
				if x and y and mapID then
					...
					local waypointMapID, waypointX, waypointY = RQE.API.GetNextWaypoint(RQE.CurrentTrackedQuestID)
...
function RQE:KhadgarCoordsMatch(questID)
	if not RQE.API.IsAddOnLoaded("RQE_Contribution") then return end
...
function RQE:NoDBEntryForQuest(questID)
	if RQE.API.IsAddOnLoaded("Chattynator") then return end
	if not RQE.API.IsAddOnLoaded("RQE_Contribution") then return end
...
function RQE.Buttons.CQButton(RQEQuestFrame)
	...
	-- 	-- Code for showing all quests functionality here
...
-- Parent function to Create SCButton
function RQE.Buttons.SCButton(RQEQuestFrame)
	...
	-- This button is intentionally inert until its behavior is added later.
	-- SCButton:SetScript("OnClick", function()
	-- 	-- No function linked yet.
	-- end)
	-- This button displays every active quest whose listed objectives are all complete.
	...
	CreateTooltip(SCButton, "Show All Completed Quests")  -- Tooltip
