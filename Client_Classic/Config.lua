...
-- Render the custom RQE settings menu's individual font names in their own
-- font.  Blizzard's standard AceConfig select control does not expose this.
do
...
function RQE:AddFontSettingsWidgets(container)
	...
	-- Creates a shared Font Style dropdown.  The open menu and selected value
	-- use the selected typeface so players can judge it before applying it.
	local function CreateFontStyleDropdown(group, profilePath)
		...
		local style = RQE.FontStyleOptions[styleKey]
		local previewItemType = AceGUI:GetWidgetVersion("Dropdown-Item-RQEFontPreview") and "Dropdown-Item-RQEFontPreview" or nil

		dropdown:SetLabel("Font Style")
		dropdown:SetList(RQE.FontStyleLabels, RQE.FontStyleOrder, previewItemType)
		dropdown:SetValue(styleKey)
		dropdown:SetFullWidth(true)
		dropdown:SetPulloutWidth(250)
		if style then
			dropdown.text:SetFont(style.path, 13, "OUTLINE")
		end
		dropdown:SetCallback("OnValueChanged", function(widget, event, value)
			local selectedStyle = RQE.FontStyleOptions[value]
			RQE.db.profile.textSettings[profilePath].font = RQE:GetFontStylePath(value)
			if selectedStyle then
				widget.text:SetFont(selectedStyle.path, 13, "OUTLINE")
			end
			RQE:ConfigurationChanged()
		end)
		group:AddChild(dropdown)
